    ...
    <div v-else class="repository-page page-shell">
      ...
      <section class="repository-overview panel">
        ...
        <div>
          ...
          <p>仓库由受信 Worker 管理。页面只持有稳定 Repository Key、Branch、Commit 与审计摘要。</p>
        </div>
      ...
      <StatePanel
        ...
        :description="store.state.repositoryCatalog.errorMessage ?? undefined"
        @retry="reload"
      />

      <section v-if="showCreate" class="create-binding panel" aria-labelledby="create-binding-title" @keydown.esc="closeCreate">
        <div class="panel-heading">
          <div><p class="eyebrow">New binding</p><h2 id="create-binding-title">绑定受管仓库</h2><p>Repository Key 来自服务端 Catalog，默认 Branch 在创建前完成 Preflight。</p></div>
        ...
        <form class="binding-form" @submit.prevent="createBinding">
          <label>Repository Key
            <select ref="createRepositorySelect" v-model="repositoryKey" :aria-invalid="submitted && !repositoryKey">
              <option value="" disabled>选择受管仓库</option>
...
@media (max-width: 767px) { .repository-overview { grid-template-columns: 44px 1fr; padding: 17px; }.repository-overview > i { width: 44px; height: 44px; }.repository-overview > :last-child { grid-column: 1 / -1; justify-self: start; }.binding-form { grid-template-columns: 1fr; padding: 16px; }.form-actions { display: grid; grid-template-columns: 1fr 1fr; }.field-message { grid-column: 1; }.binding-card { grid-template-columns: 1fr; gap: 13px; padding: 16px; }.binding-actions { grid-column: 1; grid-row: auto; justify-content: stretch; }.binding-actions > * { flex: 1; }.binding-facts { grid-column: 1; grid-template-columns: 1fr 1fr; }.preflight-result { grid-column: 1; margin: 0; }.panel-heading { flex-wrap: wrap; }.directory-actions { width: 100%; }.directory-actions > * { flex: 1; } }
</style>
