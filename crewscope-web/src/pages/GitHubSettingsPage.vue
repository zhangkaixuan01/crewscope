...
import { CheckCircle2, GitFork, HeartPulse, KeyRound, Link2, Plus, RefreshCw, ShieldCheck, TriangleAlert } from '@lucide/vue'
...
import type { DeliveryScope, GitHubAuthorizationHealth, GitHubConnection, GitHubRepository } from '../domains/delivery/types'
  ...
  <AppShell title="GitHub 集成" eyebrow="Settings · Provider connections">
    ...
    <div v-else class="github-page page-shell">
      ...
      <section v-if="selectedConnection" class="panel connection-detail">
        ...
        <div class="catalog-heading"><div><h3>Repository Catalog</h3><p>来自该 Connection 的远程可访问仓库，用于 GitHub Delivery（Push / Draft PR）。它不会直接出现在 WorkProject 的受管本地 RepositoryBinding 下拉框中。</p></div><BaseButton variant="secondary" size="small" :loading="syncing" :disabled="!online" @click="synchronize"><RefreshCw :size="14" />同步 Catalog</BaseButton></div>
        ...
        <ul v-else class="repository-list"><li v-for="repository in repositories" :key="repository.externalRepositoryId"><Link2 :size="15" /><span><strong>{{ repository.fullName }}</strong><small>{{ repository.visibility }} · 默认分支 {{ repository.defaultBranch }}</small></span></li></ul>
      </section>
    </div>
  </AppShell>
</template>
