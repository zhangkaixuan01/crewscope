...
/** `canOpenCreate` 合并了「未选 Team」与「Provider 目录未就绪」两个成因，这里把它们拆开，让禁用态能分别说明。 */
const teamMissingReason = computed(() => (scopeStore.state.selectedTeamId ? '' : '请先选择 Team，模型连接属于明确的 Organization 与 Team。'))
const providerCatalogReason = computed(() => {
  if (modelStore.state.providers.phase === 'error') return 'Provider 目录读取失败，请刷新后重试。'
  if (modelStore.state.providers.phase === 'ready' && providers.value.length === 0) return '平台还没有启用任何 Provider，暂时无法创建连接。'
  return ''
})
const createConnectionReason = computed(() => teamMissingReason.value || providerCatalogReason.value)
...
<template>
  <SettingsShell eyebrow="设置 · 模型治理" :title="`${team?.name ?? 'Team'} · 模型与凭证`">
    <template #actions><BaseButton size="small" :disabled="!canOpenCreate" :aria-describedby="createConnectionReason ? 'model-create-reason' : undefined" @click="openCreate"><Plus :size="14" />创建连接</BaseButton></template>

    <p id="model-create-reason" class="sr-only">{{ createConnectionReason }}</p>
    ...
    <StatePanel v-else-if="!scopeStore.state.selectedTeamId" state="empty" title="请选择 Team" description="模型连接与凭证始终属于明确的 Organization 与 Team。" />
    <div v-else class="model-page page-shell">
      <section class="model-overview panel" aria-labelledby="model-overview-title">
        ...
        <BaseButton class="mobile-create" size="small" :disabled="!canOpenCreate" :aria-describedby="createConnectionReason ? 'model-create-reason' : undefined" @click="openCreate"><Plus :size="14" />创建连接</BaseButton>
      </section>
      ...
      <section class="connections-panel panel" aria-labelledby="connections-title">
        ...
        <StatePanel v-else-if="connectionResource.phase === 'empty'" state="empty" compact :title="`${ownerLabel(activeOwnerType)}还没有模型连接`" description="创建后需验证为 HEALTHY，才能进入 Agent 模型选择。"><template v-if="canOpenCreate" #action><BaseButton size="small" @click="openCreate"><Plus :size="14" />创建连接</BaseButton></template></StatePanel>
        <ul v-else class="connection-list" role="list">
          <li v-for="connection in connections" :key="connection.id">
            <button type="button" :data-connection-id="connection.id" :class="{ selected: selection.connectionId === connection.id }" @click="selectConnection(connection)">
              ...
              <span class="connection-meta"><small>费用 {{ enumLabel(connection.billingSubjectType, modelSubjectTypeLabels) }}</small><small>Failures {{ connection.consecutiveFailures }}</small><small>Version {{ connection.version }}</small></span>
            </button>
          </li>
        </ul>
      </section>
...
<style scoped>
.model-overview { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: var(--cs-space-24); padding: var(--cs-space-20); background: linear-gradient(135deg, var(--cs-surface), var(--cs-surface-accent)); }.model-overview > div:first-child { display: flex; align-items: center; gap: var(--cs-space-12); }.overview-icon { display: grid; width: 48px; height: 48px; place-items: center; border-radius: 14px; background: var(--cs-surface-accent-strong); color: var(--cs-text-brand); }.model-overview h2 { margin-bottom: var(--cs-space-4); font-size: var(--cs-text-lg); }.model-overview p:last-child { margin: 0; color: var(--cs-text-muted); font-size: var(--cs-text-sm); }.model-overview dl { display: grid; grid-template-columns: repeat(3, minmax(85px, 1fr)); gap: var(--cs-space-8); margin: 0; }.model-overview dl div { padding: var(--cs-space-12) var(--cs-space-12); border: 1px solid var(--cs-border); border-radius: var(--cs-radius-sm); background: var(--cs-surface-glass); }.model-overview dt { color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.model-overview dd { margin-top: var(--cs-space-4); font: var(--cs-weight-semibold) var(--cs-text-lg) var(--cs-font-display); }.mobile-create { display: none; }.catalog-layout { display: grid; grid-template-columns: 220px minmax(0, 1fr); min-height: 250px; border-top: 1px solid var(--cs-border); }.provider-list { padding: var(--cs-space-12); border-right: 1px solid var(--cs-border); background: var(--cs-surface-subtle); }.provider-list button { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: var(--cs-space-8); width: 100%; padding: var(--cs-space-12); border: 1px solid transparent; border-radius: var(--cs-radius-sm); background: transparent; text-align: left; cursor: pointer; }.provider-list button.active { border-color: var(--cs-border-accent); background: var(--cs-surface); box-shadow: var(--cs-shadow-hairline); }.provider-list strong, .provider-list small { display: block; }.provider-list strong { font-size: var(--cs-text-sm); }.provider-list small { color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.provider-list button > small { grid-column: 1 / -1; }.catalog-content { min-width: 0; padding: var(--cs-space-12); }.provider-policy { display: flex; flex-wrap: wrap; gap: var(--cs-space-8); margin-bottom: var(--cs-space-12); }.provider-policy span { padding: var(--cs-space-4) var(--cs-space-8); border-radius: 6px; background: var(--cs-surface-accent); color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.provider-policy strong { color: var(--cs-text-secondary); }.model-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: var(--cs-space-8); margin: 0; padding: 0; list-style: none; }.model-grid li { display: grid; gap: var(--cs-space-12); padding: var(--cs-space-12); border: 1px solid var(--cs-border); border-radius: var(--cs-radius-md); background: var(--cs-surface); }.model-grid header { display: flex; justify-content: space-between; gap: var(--cs-space-8); }.model-grid h3 { margin: 0; font-size: var(--cs-text-sm); }.model-grid header p { margin: var(--cs-space-2) 0 0; color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.model-grid dl { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--cs-space-4); margin: 0; }.model-grid dl div { padding: var(--cs-space-8); border-radius: 6px; background: var(--cs-surface-subtle); }.model-grid dt { color: var(--cs-text-muted); font-size: var(--cs-text-xs); text-transform: uppercase; }.model-grid dd { margin-top: var(--cs-space-2); color: var(--cs-text-secondary); font-size: var(--cs-text-xs); font-weight: var(--cs-weight-semibold); }.capabilities { margin: 0; color: var(--cs-text-brand); font-size: var(--cs-text-xs); font-weight: var(--cs-weight-semibold); }.model-grid footer { display: flex; gap: var(--cs-space-8); padding-top: var(--cs-space-8); border-top: 1px solid var(--cs-border); color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.model-grid footer svg { flex: 0 0 auto; }.owner-tabs { display: flex; gap: var(--cs-space-4); padding: 0 var(--cs-space-16) var(--cs-space-12); }.owner-tabs button { min-height: var(--cs-density-control-height); padding: 0 var(--cs-space-12); border: 1px solid var(--cs-border); border-radius: 8px; background: var(--cs-surface-subtle); color: var(--cs-text-muted); font-size: var(--cs-text-xs); font-weight: var(--cs-weight-semibold); cursor: pointer; }.owner-tabs button.active { border-color: var(--cs-border-accent-strong); background: var(--cs-surface-accent); color: var(--cs-text-brand-strong); }.connection-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: var(--cs-space-8); margin: 0; padding: 0 var(--cs-space-16) var(--cs-space-16); list-style: none; }.connection-list li { min-width: 0; }.connection-list button { display: grid; width: 100%; height: 100%; gap: var(--cs-space-12); padding: var(--cs-space-12); border: 1px solid var(--cs-border); border-radius: var(--cs-radius-md); background: var(--cs-surface); color: var(--cs-text); text-align: left; cursor: pointer; }.connection-list button:hover, .connection-list button:focus-visible, .connection-list button.selected { border-color: var(--cs-border-accent-strong); background: var(--cs-surface-accent); }.connection-provider { display: flex; align-items: center; gap: var(--cs-space-8); color: var(--cs-text-brand); }.connection-provider strong, .connection-provider small { display: block; }.connection-provider strong { color: var(--cs-text); font-size: var(--cs-text-sm); }.connection-provider small { margin-top: var(--cs-space-2); color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.connection-badges { display: flex; flex-wrap: wrap; gap: var(--cs-space-4); }.connection-meta { display: flex; flex-wrap: wrap; gap: var(--cs-space-8); padding-top: var(--cs-space-8); border-top: 1px solid var(--cs-border); color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.governance-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--cs-space-8); padding: 0 var(--cs-space-16) var(--cs-space-16); }.governance-grid article { display: grid; grid-template-columns: 20px minmax(0, 1fr) auto; align-items: start; gap: var(--cs-space-8); padding: var(--cs-space-12); border: 1px solid var(--cs-border); border-radius: var(--cs-radius-md); background: var(--cs-surface-subtle); color: var(--cs-text-brand); }.governance-grid h3 { margin: 0; color: var(--cs-text); font-size: var(--cs-text-sm); }.governance-grid p { margin: var(--cs-space-4) 0 0; color: var(--cs-text-muted); font-size: var(--cs-text-xs); line-height: var(--cs-leading-normal); }.security-note { display: flex; gap: var(--cs-space-8); padding: var(--cs-space-12) var(--cs-space-16); border: 1px solid var(--cs-border-accent); border-radius: var(--cs-radius-md); background: var(--cs-surface-accent); color: var(--cs-text-brand); }.security-note svg { flex: 0 0 auto; }.security-note strong, .security-note span { display: block; }.security-note strong { font-size: var(--cs-text-sm); }.security-note span { margin-top: var(--cs-space-2); color: var(--cs-text-secondary); font-size: var(--cs-text-xs); }
@media (max-width: 980px) { .model-overview { grid-template-columns: 1fr; }.catalog-layout { grid-template-columns: 1fr; }.provider-list { display: flex; gap: var(--cs-space-8); overflow-x: auto; border-right: 0; border-bottom: 1px solid var(--cs-border); }.provider-list button { min-width: 180px; }.governance-grid { grid-template-columns: 1fr; } }
@media (max-width: 767px) { .mobile-create { display: inline-flex; grid-column: 1 / -1; width: 100%; }.model-overview dl { width: 100%; }.connection-list { grid-template-columns: 1fr; padding-inline: var(--cs-space-12); }.owner-tabs { overflow-x: auto; padding-inline: var(--cs-space-12); }.governance-grid { padding-inline: var(--cs-space-12); } }
@media (max-width: 520px) { .model-overview { padding: var(--cs-space-16); }.model-overview > div:first-child { align-items: flex-start; }.model-overview dl { grid-template-columns: 1fr; }.model-overview dl div { display: flex; justify-content: space-between; align-items: center; padding: var(--cs-space-8) var(--cs-space-8); }.model-overview dd { margin: 0; font-size: var(--cs-text-base); }.model-grid { grid-template-columns: 1fr; }.provider-policy { display: grid; grid-template-columns: 1fr 1fr; } }
</style>
