...
import { AlertTriangle, Ban, FileQuestion, LoaderCircle, RefreshCw, Scale } from '@lucide/vue'
import { computed } from 'vue'
import BaseButton from '../base/BaseButton.vue'

const props = withDefaults(defineProps<{
  state: 'loading' | 'empty' | 'error' | 'forbidden' | 'conflict'
  title?: string
  description?: string
}>(), {
  title: '',
...
<template>
  <section class="state-panel" :aria-busy="state === 'loading'" :role="state === 'error' ? 'alert' : 'status'">
    <component :is="presentation.icon" :class="{ spinning: state === 'loading' }" :size="22" aria-hidden="true" />
    <div>
