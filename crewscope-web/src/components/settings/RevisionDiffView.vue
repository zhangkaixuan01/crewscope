...
.revision-diff header, .revision-diff__head, .revision-diff__row { display: grid; grid-template-columns: minmax(150px, .8fr) minmax(0, 1fr) minmax(0, 1fr); gap: var(--cs-space-3); align-items: start; }
.revision-diff header { grid-template-columns: 1fr auto; padding: var(--cs-space-4); border-bottom: 1px solid var(--cs-border); }
.revision-diff h3 { margin: 0; font-size: var(--cs-text-lg); }.revision-diff p { margin: var(--cs-space-1) 0 0; color: var(--cs-text-muted); font-size: var(--cs-text-xs); }
.revision-diff__head { padding: var(--cs-space-2) var(--cs-space-4); background: var(--cs-surface-subtle); color: var(--cs-text-muted); font-size: var(--cs-text-xs); font-weight: 700; }
.revision-diff__row { padding: var(--cs-space-3) var(--cs-space-4); border-top: 1px solid var(--cs-border); color: var(--cs-text-secondary); font-size: var(--cs-text-xs); word-break: break-word; }.revision-diff__row.changed { background: var(--cs-warning-soft); }.revision-diff__row code { color: var(--cs-text); font-weight: 700; }.revision-diff__empty { padding: var(--cs-space-4); color: var(--cs-text-muted); font-size: var(--cs-text-sm); }
@media (max-width: 640px) { .revision-diff__head, .revision-diff__row { grid-template-columns: 1fr; gap: var(--cs-space-1); }.revision-diff__head span:not(:first-child)::before, .revision-diff__row span::before { display: inline-block; width: 72px; color: var(--cs-text-muted); font-weight: 700; }.revision-diff__head span:nth-child(2)::before, .revision-diff__row span:nth-child(2)::before { content: '版本 A：'; }.revision-diff__head span:nth-child(3)::before, .revision-diff__row span:nth-child(3)::before { content: '版本 B：'; } }
</style>
