...
import {
  inboxCloseReasonLabels,
  inboxDispositionStatusLabels,
  inboxPriorityLabels,
  inboxSourceStatusLabels,
  inboxSourceTypeLabels,
...
/** 「全部类型 + 全部处置」是 Inbox 的两个可选筛选；来源状态是成员刻意选择的二值，不在这里重置。 */
const hasOptionalFilter = computed(() => props.itemType !== 'ALL' || props.dispositionStatus !== 'ALL')

function clearOptionalFilters(): void {
  emit('changeType', 'ALL')
  emit('changeDispositionStatus', 'ALL')
}
...
/**
 * The count as a top-level binding, because only those are unwrapped in a template.
 *
 * `selection.selectedCount` reached through the object is the ref itself — always truthy — so the
 * batch bar used to render over an empty selection with 「已选 0 项」 and a live button.
 */
...
function dispositionLabel(status: InboxDispositionStatus): string {
  return inboxDispositionStatusLabels[status]
}

/** Inbox rows quote enums owned by five different aggregates, so each reads through its own map. */
...
<template>
  <section class="inbox-workspace" aria-label="我的 Inbox">
    ...
    <section class="inbox-toolbar panel" aria-label="Inbox 筛选">
      ...
      <span>{{ items.length }} 项已加载<span v-if="selectedCount">，已选 {{ selectedCount }}</span></span>
    ...
    <div class="inbox-content" :class="{ 'has-detail': selectedItemId }">
      <section class="inbox-list panel" aria-label="Inbox 项目列表">
        ...
        <StatePanel v-else-if="forbidden" state="forbidden" title="无权读取 Inbox" description="服务端没有为当前身份解析出这个 Team 的活动成员。"><template #action><RouterLink :to="{ name: 'access-denied', query: { requiredPermission: permissions.scopeRead } }"><BaseButton variant="secondary" size="small">查看权限说明</BaseButton></RouterLink></template></StatePanel>
      ...
      <aside v-if="selectedItemId" class="inbox-detail panel" aria-label="Inbox 详情">
        ...
        <template v-else-if="selected">
          ...
          <dl>
            <div><dt>来源</dt><dd>{{ label(selected.source.type, inboxSourceTypeLabels) }}</dd></div>
            <div><dt>来源 Revision</dt><dd>{{ selected.source.revision }}</dd></div>
            <div><dt>来源状态</dt><dd>{{ label(selected.sourceStatus, inboxSourceStatusLabels) }}</dd></div>
            ...
            <div v-if="selected.closeReason"><dt>关闭原因</dt><dd>{{ label(selected.closeReason, inboxCloseReasonLabels) }}</dd></div>
          </dl>
...
@media (max-width: 760px) { .inbox-overview { grid-template-columns: 38px 1fr; padding: var(--cs-space-12); }.inbox-overview > span { width: 38px; height: 38px; }.inbox-overview > :last-child { grid-column: 1 / -1; justify-self: start; }.inbox-views { display: flex; overflow-x: auto; padding-bottom: var(--cs-space-2); scroll-snap-type: x proximity; }.inbox-views button { flex: 0 0 160px; scroll-snap-align: start; }.inbox-toolbar { grid-template-columns: 1fr 1fr; }.inbox-toolbar > span { grid-column: 1 / -1; justify-self: start; padding: 0; }.inbox-content.has-detail { grid-template-columns: 1fr; }.inbox-detail { position: static; grid-row: 1; }.inbox-list article { padding: var(--cs-space-12); } }
@media (max-width: 460px) { .inbox-toolbar { grid-template-columns: 1fr; }.inbox-toolbar > span { grid-column: auto; }.inbox-detail > dl { grid-template-columns: 1fr; }.inbox-detail__source { grid-template-columns: 18px minmax(0, 1fr); }.inbox-detail__source > button { grid-column: 1 / -1; width: 100%; }.inbox-detail__actions > div { display: grid; }.inbox-detail__actions > div > * { width: 100%; } }
</style>
