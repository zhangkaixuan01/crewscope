...
withDefaults(defineProps<{
  ...
  canDelegate?: boolean
}>(), { errorMessage: null, canDelegate: false })

const emit = defineEmits<{
  open: [association: ConversationWorkItemAssociation]
  delegate: [association: ConversationWorkItemAssociation]
  retry: []
}>()
  ...
  >
    ...
    <ul v-else>
      <li v-for="association in associations" :key="association.linkId">
        ...
        <BaseButton
          v-if="direction === 'conversation' && association.origin === 'TASK_INTENT_CONFIRMATION' && canDelegate"
          variant="secondary"
          size="small"
          :aria-label="`为工作项 ${association.workItem.key} 配置 Coding Task`"
          @click="emit('delegate', association)"
        >配置 Coding Task</BaseButton>
      </li>
    </ul>
  </section>
</template>
...
<style scoped>
.association-card { max-width: 740px; padding: var(--cs-space-16); margin: 0 auto var(--cs-space-16); border: 1px solid var(--cs-border); border-radius: var(--cs-radius-md); background: linear-gradient(135deg, var(--cs-surface) 0%, var(--cs-surface-accent) 100%); box-shadow: var(--cs-shadow-raised); }
.association-card > header { display: grid; grid-template-columns: 32px 1fr auto; align-items: center; gap: var(--cs-space-8); margin-bottom: var(--cs-space-12); }.association-card > header > span { display: grid; width: 32px; height: 32px; place-items: center; border-radius: 10px; background: var(--cs-surface-accent-strong); color: var(--cs-text-brand); }.association-card header p { margin: 0 0 var(--cs-space-2); color: var(--cs-text-brand); font-size: var(--cs-text-xs); font-weight: var(--cs-weight-semibold); letter-spacing: .07em; text-transform: uppercase; }.association-card h3 { margin: 0; font-size: var(--cs-text-base); }
.association-card ul { display: grid; gap: var(--cs-space-8); padding: 0; margin: 0; list-style: none; }.association-card li { display: grid; grid-template-columns: 30px minmax(0, 1fr) auto auto auto; align-items: center; gap: var(--cs-space-8); padding: var(--cs-space-8); border: 1px solid var(--cs-border); border-radius: var(--cs-radius-sm); background: var(--cs-surface-glass); }.association-icon { display: grid; width: 30px; height: 30px; place-items: center; border-radius: 9px; background: var(--cs-surface-subtle); color: var(--cs-text-brand); }.association-card li strong, .association-card li div > span, .association-card li small { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.association-card li strong { color: var(--cs-text); font-size: var(--cs-text-sm); }.association-card li div > span { margin-top: var(--cs-space-2); color: var(--cs-text-secondary); font-size: var(--cs-text-xs); }.association-card li small { margin-top: var(--cs-space-2); color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.association-state { display: flex; min-height: 42px; align-items: center; justify-content: center; gap: var(--cs-space-8); margin: 0; color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.association-state.error { color: var(--cs-danger); }.association-state button { border: 0; background: transparent; color: inherit; font-weight: var(--cs-weight-semibold); text-decoration: underline; cursor: pointer; }.spin { animation: spin var(--cs-motion-spin) linear infinite; }
