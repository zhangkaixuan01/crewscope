...
import {
  inboxItemTypeLabels,
  larkConnectionStatusLabels,
  larkCredentialStatusLabels,
  larkHealthStatusLabels,
  larkMappingStatusLabels,
  larkMappingTerminalReasonLabels,
  notificationDeliveryStatusLabels,
  notificationFailureCodeLabels,
...
/** This admin surface quotes enums from the Lark connection, mapping, inbox and notification aggregates. */
...
// 禁用态必须可解释。`canMutate` 是「在线且没有命令在途」的锁，成员无从得知原因，因此由一条
// 共享原因承载；其余业务前置条件各自给出具名原因。原因统一用 aria-describedby 关联到控件，
// 而不是只把文字放在旁边——后者对读屏不可达。
const hasActiveConnection = computed(() => props.connections.some(item => item.status === 'ACTIVE'))
...
const mutateReason = computed(() => (canMutate.value
  ? null
  : props.online ? '命令正在提交，完成前无法发起新的变更。' : '当前离线，联网后才能提交变更。'))
...
/** 两个历史表格的「当前筛选没有…」空状态：把筛选恢复为全部并重新查询，才是成员能执行的下一步。 */
const hasMappingFilter = computed(() => localMappingStatus.value !== '')
const hasDeliveryFilter = computed(() => Boolean(deliveryFilter.status || deliveryFilter.itemType || deliveryFilter.recipient.trim()))

function clearMappingFilter(): void {
  localMappingStatus.value = ''
  applyMappingFilter()
}

function clearDeliveryFilter(): void {
  deliveryFilter.status = ''
  deliveryFilter.itemType = ''
  deliveryFilter.recipient = ''
  applyDeliveryFilter()
}
...
<template>
  <section class="lark-admin" aria-labelledby="lark-admin-title">
    ...
    <p id="lark-mutate-reason" class="sr-only">{{ mutateReason }}</p>
    <p id="lark-verify-reason" class="sr-only">需要有效的 Provider Binding、已选择成员并填入精确 open_id 才能验证身份。</p>
    <p id="lark-confirm-reason" class="sr-only">先完成上一步身份验证，通过后才能确认映射。</p>
    <p id="lark-preference-reason" class="sr-only">至少保留一种通知类型才能保存偏好。</p>
    ...
    <template v-if="selectedTab === 'connection'">
      <div class="connection-grid">
        <section class="panel connection-list" aria-label="飞书连接列表">
          <header><div><p>Credential boundary</p><h3>Team Connection</h3></div><BaseButton size="small" :disabled="!canMutate || hasActiveConnection" :aria-describedby="hasActiveConnection ? 'lark-create-reason' : (canMutate ? undefined : 'lark-mutate-reason')" @click="openCredentialDialog('create')"><KeyRound :size="13" />创建连接</BaseButton></header>
          <p v-if="hasActiveConnection" id="lark-create-reason" class="sr-only">已有 ACTIVE 连接，需要先撤销当前连接才能新建。</p>
          <StatePanel v-if="phase === 'loading'" state="loading" title="正在加载飞书连接" />
          <StatePanel v-else-if="error?.kind === 'forbidden'" state="forbidden" title="无权管理 Provider"><template #action><RouterLink :to="{ name: 'access-denied', query: { requiredPermission: permissions.providerManage } }"><BaseButton variant="secondary" size="small">查看权限说明</BaseButton></RouterLink></template></StatePanel>
          ...
          <StatePanel v-else-if="connections.length === 0" state="empty" title="尚未连接飞书" description="使用 Team 的 App 凭证创建一条单向 Secret 连接。"><template v-if="canMutate" #action><BaseButton size="small" @click="openCredentialDialog('create')"><KeyRound :size="15" />创建连接</BaseButton></template></StatePanel>
          <button v-for="item in connections" v-else :key="item.connectionId" class="connection-row" type="button" :class="{ selected: item.connectionId === selectedConnection?.connectionId }" @click="emit('selectConnection', item.connectionId)">
            <span><strong>{{ item.maskedAppId }}</strong><small class="mono">{{ shortId(item.connectionId) }}</small></span><StatusBadge :tone="connectionTone(item.status)">{{ label(item.status, larkConnectionStatusLabels) }}</StatusBadge><small>v{{ item.version }}</small>
          </button>
        </section>
        ...
        <section class="panel connection-detail" aria-label="飞书连接详情">
          ...
          <template v-else>
            <header><div><p>Public metadata only</p><h3>{{ selectedConnection.maskedAppId }}</h3></div><StatusBadge :tone="connectionTone(selectedConnection.status)">{{ label(selectedConnection.status, larkConnectionStatusLabels) }}</StatusBadge></header>
            <dl><div><dt>Binding</dt><dd class="mono">{{ selectedConnection.providerBindingId ?? '未创建' }}</dd></div><div><dt>Binding 强版本</dt><dd class="mono">{{ selectedConnection.providerBindingVersion == null ? '—' : `v${selectedConnection.providerBindingVersion}` }}</dd></div><div><dt>Credential</dt><dd>{{ label(selectedConnection.credentialStatus, larkCredentialStatusLabels) }}</dd></div><div><dt>Credential 强版本</dt><dd class="mono">v{{ selectedConnection.version }}</dd></div><div><dt>过期时间</dt><dd>{{ displayTime(selectedConnection.expiresAt) }}</dd></div><div><dt>更新时间</dt><dd>{{ displayTime(selectedConnection.updatedAt) }}</dd></div></dl>
            <section v-if="health" class="health"><span><ShieldCheck :size="18" /></span><div><strong>安全健康</strong><small>{{ health.evidenceCode }} · {{ displayTime(health.checkedAt) }}</small></div><StatusBadge :tone="healthTone(health.status)">{{ label(health.status, larkHealthStatusLabels) }}</StatusBadge></section>
            <footer>
              <BaseButton variant="secondary" size="small" :disabled="!canMutate || !selectedConnection.providerBindingId || selectedConnection.providerBindingVersion == null" :aria-describedby="(!selectedConnection.providerBindingId || selectedConnection.providerBindingVersion == null) ? 'lark-binding-reason' : (canMutate ? undefined : 'lark-mutate-reason')" @click="selectedConnection.providerBindingId && selectedConnection.providerBindingVersion != null && emit('preflight', selectedConnection.providerBindingId, selectedConnection.providerBindingVersion)"><Link2 :size="13" />Preflight</BaseButton>
              <BaseButton variant="secondary" size="small" :disabled="!online || !selectedConnection.providerBindingId" :aria-describedby="!selectedConnection.providerBindingId ? 'lark-binding-reason' : 'lark-mutate-reason'" @click="selectedConnection.providerBindingId && emit('health', selectedConnection.providerBindingId)"><RefreshCw :size="13" />健康检查</BaseButton>
              <BaseButton variant="secondary" size="small" :disabled="!canMutate || selectedConnection.status !== 'ACTIVE'" :aria-describedby="selectedConnection.status !== 'ACTIVE' ? 'lark-rotate-reason' : (canMutate ? undefined : 'lark-mutate-reason')" @click="openCredentialDialog('rotate')"><RotateCw :size="13" />轮换凭证</BaseButton>
              <p id="lark-binding-reason" class="sr-only">需要先创建 Provider Binding 才能执行这个操作。</p>
              <p id="lark-rotate-reason" class="sr-only">只有 ACTIVE 状态的连接可以轮换凭证。</p>
            </footer>
            <form v-if="selectedConnection.status === 'ACTIVE'" class="danger-zone" @submit.prevent="emit('revokeConnection', selectedConnection.connectionId, revokeReason, newKey())"><label>撤销原因<input v-model.trim="revokeReason" maxlength="500" autocomplete="off" required></label><BaseButton variant="danger" size="small" type="submit" :disabled="!canMutate || !revokeReason" :aria-describedby="!revokeReason ? 'lark-revoke-reason' : (canMutate ? undefined : 'lark-mutate-reason')"><Unplug :size="13" />撤销</BaseButton></form>
            <p v-if="!revokeReason" id="lark-revoke-reason" class="sr-only">请先填写撤销原因。</p>
          </template>
        </section>
      </div>
    </template>
    ...
    <template v-else-if="selectedTab === 'mapping'">
      <section class="mapping-flow panel">
        ...
        <div class="mapping-steps">
          ...
          <form @submit.prevent="verify"><label><span>2 · 精确飞书 open_id</span><input v-model="openId" type="password" maxlength="200" autocomplete="off" placeholder="仅用于本次验证" required></label><BaseButton size="small" type="submit" :disabled="!canMutate || verifyPrerequisitesMissing" :aria-describedby="verifyPrerequisitesMissing ? 'lark-verify-reason' : (canMutate ? undefined : 'lark-mutate-reason')"><UserCheck :size="13" />验证身份</BaseButton></form>
          <div class="proof"><span>3 · Proof Receipt</span><strong v-if="verified" class="mono">{{ shortId(verified.proofId) }}</strong><small v-else>验证通过后生成一次性安全坐标</small><BaseButton size="small" :disabled="!canMutate || !verified" :aria-describedby="!verified ? 'lark-confirm-reason' : (canMutate ? undefined : 'lark-mutate-reason')" @click="confirm"><UsersRound :size="13" />确认映射</BaseButton></div>
        </div>
      </section>
      <section class="panel table-panel" aria-label="成员映射历史">
        <header><div><p>Team directory</p><h3>成员映射</h3></div><form @submit.prevent="applyMappingFilter"><select v-model="localMappingStatus" aria-label="成员映射状态"><option value="">全部状态</option><option v-for="status in larkMappingStatuses" :key="status" :value="status">{{ larkMappingStatusLabels[status] }}</option></select><BaseButton variant="secondary" size="small" type="submit">筛选</BaseButton></form></header>
        <StatePanel v-if="mappingPhase === 'loading'" state="loading" />
        <StatePanel v-else-if="mappingError?.kind === 'forbidden'" state="forbidden"><template #action><RouterLink :to="{ name: 'access-denied', query: { requiredPermission: permissions.providerManage } }"><BaseButton variant="secondary" size="small">查看权限说明</BaseButton></RouterLink></template></StatePanel>
        <StatePanel v-else-if="mappingInitialFailure" state="error" :description="mappingError?.message" @retry="applyMappingFilter" />
        <StatePanel v-else-if="mappings.length === 0" state="empty" title="当前筛选没有成员映射"><template v-if="hasMappingFilter" #action><BaseButton variant="secondary" size="small" @click="clearMappingFilter">清空筛选</BaseButton></template></StatePanel>
        <template v-else><StatePanel v-if="!online" compact state="offline" title="正在展示最近同步的成员映射" /><StatePanel v-else-if="mappingError?.kind === 'cursor-expired'" compact state="error" title="成员映射 Cursor 已过期" description="已加载映射保持可读，刷新首屏后可继续。" @retry="applyMappingFilter" /><StatePanel v-else-if="mappingError" compact state="error" :description="mappingError.message" @retry="applyMappingFilter" /><table><thead><tr><th>成员</th><th>Binding</th><th>状态</th><th>验证时间</th><th>版本</th><th></th></tr></thead><tbody><tr v-for="item in mappings" :key="item.mappingId"><td class="mono" data-label="成员">{{ shortId(item.memberId) }}</td><td class="mono" data-label="Binding">{{ shortId(item.providerBindingId) }}</td><td data-label="状态"><StatusBadge :tone="mappingTone(item.status)">{{ label(item.status, larkMappingStatusLabels) }}</StatusBadge><small v-if="item.terminalReason">{{ label(item.terminalReason, larkMappingTerminalReasonLabels) }}</small></td><td data-label="验证时间">{{ displayTime(item.verifiedAt) }}</td><td class="mono" data-label="版本">v{{ item.version }}</td><td data-label="操作"><BaseButton v-if="item.status === 'ACTIVE'" variant="ghost" size="small" :disabled="!canMutate" :aria-describedby="canMutate ? undefined : 'lark-mutate-reason'" @click="emit('revokeMapping', item.mappingId, item.version, 'ADMIN_REVOKED', newKey())">撤销</BaseButton></td></tr></tbody></table><footer v-if="mappingNextCursor"><BaseButton variant="secondary" size="small" :loading="mappingLoadingMore" :disabled="!online" @click="emit('loadMoreMappings')">加载更多</BaseButton></footer></template>
      </section>
    </template>
      ...
      <div class="notification-grid">
        <section class="panel preference">
          ...
          <form v-else @submit.prevent="savePreference">
            ...
            <fieldset><legend>通知类型</legend><label v-for="type in inboxItemTypes" :key="type"><input type="checkbox" :checked="preferenceForm.enabledItemTypes.includes(type)" @change="toggleItemType(type)">{{ inboxItemTypeLabels[type] }}</label></fieldset>
            ...
            <BaseButton size="small" type="submit" :disabled="!canMutate || preferenceForm.enabledItemTypes.length === 0" :aria-describedby="preferenceForm.enabledItemTypes.length === 0 ? 'lark-preference-reason' : (canMutate ? undefined : 'lark-mutate-reason')">保存偏好</BaseButton>
          </form>
        </section>
        <section class="panel delivery-list" aria-label="通知投递历史">
          <header><div><p>Receipt & recovery</p><h3>通知投递历史</h3></div></header>
          <form class="delivery-filter" @submit.prevent="applyDeliveryFilter"><select v-model="deliveryFilter.status" aria-label="投递状态"><option value="">全部状态</option><option v-for="status in notificationDeliveryStatuses" :key="status" :value="status">{{ notificationDeliveryStatusLabels[status] }}</option></select><select v-model="deliveryFilter.itemType" aria-label="通知类型"><option value="">全部类型</option><option v-for="type in inboxItemTypes" :key="type" :value="type">{{ inboxItemTypeLabels[type] }}</option></select><select v-model="deliveryFilter.recipient" aria-label="收件成员"><option value="">全部成员</option><option v-for="member in members" :key="member.id" :value="member.id">{{ member.id === currentMemberId ? '当前成员 · ' : '' }}{{ member.displayName }}</option></select><BaseButton variant="secondary" size="small" type="submit">筛选</BaseButton></form>
          <StatePanel v-if="deliveryPhase === 'loading'" state="loading" />
          <StatePanel v-else-if="deliveryError?.kind === 'forbidden'" state="forbidden"><template #action><RouterLink :to="{ name: 'access-denied', query: { requiredPermission: permissions.providerManage } }"><BaseButton variant="secondary" size="small">查看权限说明</BaseButton></RouterLink></template></StatePanel>
          <StatePanel v-else-if="deliveryInitialFailure" state="error" :description="deliveryError?.message" @retry="applyDeliveryFilter" />
          <StatePanel v-else-if="deliveries.length === 0" state="empty" title="当前筛选没有投递事实"><template v-if="hasDeliveryFilter" #action><BaseButton variant="secondary" size="small" @click="clearDeliveryFilter">清空筛选</BaseButton></template></StatePanel>
          <template v-else><StatePanel v-if="!online" compact state="offline" title="正在展示最近同步的通知投递" /><StatePanel v-else-if="deliveryError?.kind === 'cursor-expired'" compact state="error" title="通知投递 Cursor 已过期" description="已加载投递事实保持可读，刷新首屏后可继续。" @retry="applyDeliveryFilter" /><StatePanel v-else-if="deliveryError" compact state="error" :description="deliveryError.message" @retry="applyDeliveryFilter" /><button v-for="item in deliveries" :key="item.deliveryId" class="delivery-row" type="button" :class="{ selected: item.deliveryId === selectedDelivery?.deliveryId }" @click="emit('selectDelivery', item.deliveryId)"><span><strong>{{ label(item.itemType, inboxItemTypeLabels) }}</strong><small>{{ displayTime(item.updatedAt) }} · 尝试 {{ item.attemptCount }} 次</small></span><StatusBadge :tone="deliveryTone(item.status)">{{ label(item.status, notificationDeliveryStatusLabels) }}</StatusBadge><span class="mono">{{ shortId(item.deliveryId) }}</span></button><footer v-if="deliveryNextCursor"><BaseButton variant="secondary" size="small" :loading="deliveryLoadingMore" :disabled="!online" @click="emit('loadMoreDeliveries')">加载更多</BaseButton></footer></template>
        </section>
        <aside v-if="selectedDelivery" class="panel delivery-detail" aria-label="通知投递详情"><header><div><p>Safe delivery fact</p><h3>投递详情</h3></div><button type="button" aria-label="关闭投递详情" @click="emit('closeDelivery')"><X :size="17" /></button></header><div class="delivery-result"><StatusBadge :tone="deliveryTone(selectedDelivery.status)">{{ label(selectedDelivery.status, notificationDeliveryStatusLabels) }}</StatusBadge><strong>{{ label(selectedDelivery.itemType, inboxItemTypeLabels) }}</strong><small>{{ selectedDelivery.evidenceCode ?? '暂无终态证据' }}</small></div><dl><div><dt>Delivery</dt><dd class="mono">{{ selectedDelivery.deliveryId }}</dd></div><div><dt>Recipient</dt><dd class="mono">{{ selectedDelivery.recipientMemberId }}</dd></div><div><dt>Binding</dt><dd class="mono">{{ selectedDelivery.providerBindingId }}</dd></div><div><dt>Template</dt><dd class="mono">{{ shortId(selectedDelivery.template.templateId) }} · v{{ selectedDelivery.template.version }}</dd></div><div><dt>Failure</dt><dd>{{ selectedDelivery.failureCode ? label(selectedDelivery.failureCode, notificationFailureCodeLabels) : '—' }}</dd></div><div><dt>Redelivery of</dt><dd class="mono">{{ selectedDelivery.redeliveryOf ?? '—' }}</dd></div><div><dt>版本</dt><dd class="mono">v{{ selectedDelivery.version }}</dd></div></dl><footer><BaseButton :disabled="!canMutate || !failedDelivery" :aria-describedby="!failedDelivery ? 'lark-redeliver-reason' : (canMutate ? undefined : 'lark-mutate-reason')" @click="emit('redeliver', selectedDelivery.deliveryId, newKey())"><Send :size="13" />再次投递</BaseButton><small v-if="!failedDelivery" id="lark-redeliver-reason">只有「{{ notificationDeliveryStatusLabels.FAILED_FINAL }}」的投递可由管理员显式重投。</small></footer></aside>
      </div>
    </template>
...
<style scoped>
.lark-admin{display:grid;max-width:1280px;gap:var(--cs-space-12);margin:0 auto}.hero{display:grid;grid-template-columns:44px minmax(0,1fr) auto;align-items:center;gap:var(--cs-space-12);padding:var(--cs-space-16) var(--cs-space-20)}.hero>span{display:grid;width:44px;height:44px;place-items:center;border-radius:13px;background: var(--cs-surface-accent-strong);color: var(--cs-text-brand)}.hero p,.panel header p,.credential-dialog header p{margin:0;color: var(--cs-text-brand);font-size:var(--cs-text-xs);font-weight:var(--cs-weight-semibold);letter-spacing:.1em;text-transform:uppercase}.hero h2{margin:var(--cs-space-2) 0;font-size:var(--cs-text-lg)}.hero small{color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.tabs{display:flex;padding:var(--cs-space-4)}.tabs button{min-height:36px;padding:0 var(--cs-space-16);border-radius:8px;color: var(--cs-text-muted);font-size:var(--cs-text-sm);font-weight:var(--cs-weight-semibold);cursor:pointer}.tabs button.active{background: var(--cs-surface-accent-strong);color: var(--cs-text-brand-strong)}.receipt{display:flex;align-items:center;gap:var(--cs-space-8);padding:var(--cs-space-12) var(--cs-space-12);border: 1px solid var(--cs-border-accent);border-radius:var(--cs-radius-sm);background: var(--cs-surface-accent);color: var(--cs-success)}.receipt span,.receipt strong,.receipt small{display:block}.receipt strong{font-size:var(--cs-text-xs)}.receipt small{margin-top:var(--cs-space-2);color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.connection-grid,.notification-grid{display:grid;grid-template-columns:minmax(300px,.72fr) minmax(0,1.28fr);align-items:start;gap:var(--cs-space-12)}.panel>header{display:flex;align-items:center;justify-content:space-between;gap:var(--cs-space-12);padding:var(--cs-space-12) var(--cs-space-16);border-bottom: 1px solid var(--cs-border)}.panel>header h3{margin:var(--cs-space-2) 0 0;font-size:var(--cs-text-base)}.connection-row,.delivery-row{display:grid;width:100%;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:var(--cs-space-12);padding:var(--cs-space-12) var(--cs-space-16);border-bottom: 1px solid var(--cs-border);text-align:left;cursor:pointer}.connection-row:hover,.connection-row.selected,.delivery-row:hover,.delivery-row.selected{background: var(--cs-surface-accent)}.connection-row span strong,.connection-row span small,.delivery-row span strong,.delivery-row span small{display:block}.connection-row span small,.delivery-row small{margin-top:var(--cs-space-4);color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.connection-row>small{font-size:var(--cs-text-xs)}.connection-detail>dl,.delivery-detail>dl{display:grid;grid-template-columns:1fr 1fr;gap:0 var(--cs-space-12);margin:0;padding:var(--cs-space-8) var(--cs-space-16)}.connection-detail dl div,.delivery-detail dl div{padding:var(--cs-space-8) 0;border-bottom: 1px solid var(--cs-border)}dt{color: var(--cs-text-muted);font-size:var(--cs-text-xs);font-weight:var(--cs-weight-semibold);text-transform:uppercase}dd{overflow-wrap:anywhere;margin:var(--cs-space-4) 0 0;font-size:var(--cs-text-xs)}.health{display:grid;grid-template-columns:30px 1fr auto;align-items:center;gap:var(--cs-space-8);margin:var(--cs-space-8) var(--cs-space-16);padding:var(--cs-space-12);border: 1px solid var(--cs-border);border-radius:9px;background: var(--cs-surface-subtle)}.health>span{display:grid;width:30px;height:30px;place-items:center;border-radius:8px;background: var(--cs-surface-accent-strong);color: var(--cs-text-brand)}.health strong,.health small{display:block}.health strong{font-size:var(--cs-text-xs)}.health small{margin-top:var(--cs-space-2);color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.connection-detail>footer{display:flex;flex-wrap:wrap;gap:var(--cs-space-8);padding:var(--cs-space-12) var(--cs-space-16)}.danger-zone{display:grid;grid-template-columns:1fr auto;align-items:end;gap:var(--cs-space-8);padding:var(--cs-space-12) var(--cs-space-16);border-top: 1px solid var(--cs-danger-border);background: var(--cs-danger-soft)}.danger-zone label,.credential-dialog label,.preference form>label{display:grid;gap:var(--cs-space-4);color: var(--cs-text-muted);font-size:var(--cs-text-xs);font-weight:var(--cs-weight-semibold)}input,select{min-width:0;min-height:36px;padding:0 var(--cs-space-8);border: 1px solid var(--cs-border);border-radius:8px;background: var(--cs-surface);color: var(--cs-text);font-size:var(--cs-text-base)}.mapping-flow>header{align-items:flex-start}.mapping-steps{display:grid;grid-template-columns:1fr 1.4fr 1fr;gap:var(--cs-space-12);padding:var(--cs-space-16)}.mapping-steps label,.mapping-steps form,.proof{display:grid;align-content:start;gap:var(--cs-space-8)}.mapping-steps label>span,.proof>span{color: var(--cs-text-muted);font-size:var(--cs-text-xs);font-weight:var(--cs-weight-semibold)}.mapping-steps form{grid-template-columns:1fr auto;align-items:end}.mapping-steps form label{display:grid}.proof{padding:var(--cs-space-8);border: 1px solid var(--cs-border);border-radius:9px;background: var(--cs-surface-subtle)}.proof strong,.proof small{font-size:var(--cs-text-xs)}.proof small{color: var(--cs-text-muted)}.table-panel>header form{display:flex;gap:var(--cs-space-8)}.table-panel table{width:100%;border-collapse:collapse}.table-panel th{padding:var(--cs-space-8) var(--cs-space-12);background: var(--cs-surface-subtle);color: var(--cs-text-muted);font-size:var(--cs-text-xs);text-align:left}.table-panel td{padding:var(--cs-space-12) var(--cs-space-12);border-top: 1px solid var(--cs-border);font-size:var(--cs-text-xs)}.table-panel td small{display:block;margin-top:var(--cs-space-4);color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.table-panel>footer,.delivery-list>footer{display:flex;justify-content:center;padding:var(--cs-space-12)}.notification-grid{grid-template-columns:330px minmax(0,1fr)}.notification-grid:has(.delivery-detail){grid-template-columns:290px minmax(0,1fr) 340px}.preference form{display:grid;gap:var(--cs-space-12);padding:var(--cs-space-16)}.switch{display:flex!important;align-items:center;gap:var(--cs-space-8);color: var(--cs-text)!important;font-size:var(--cs-text-sm)!important}.switch input,.preference fieldset input{min-height:0}.preference fieldset{display:grid;grid-template-columns:1fr 1fr;gap:var(--cs-space-8);padding:var(--cs-space-12);border: 1px solid var(--cs-border);border-radius:9px}.preference legend{padding:0 var(--cs-space-4);color: var(--cs-text-muted);font-size:var(--cs-text-xs);font-weight:var(--cs-weight-semibold)}.preference fieldset label{display:flex;align-items:center;gap:var(--cs-space-4);font-size:var(--cs-text-xs)}.preference form>p{margin:0;color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.preference form>:deep(.base-button){justify-self:start}.delivery-filter{display:grid;grid-template-columns:150px 135px minmax(160px,1fr) auto;gap:var(--cs-space-8);padding:var(--cs-space-12);border-bottom: 1px solid var(--cs-border)}.delivery-row>span:last-child{font-size:var(--cs-text-xs)}.delivery-detail{position:sticky;top:12px}.delivery-detail header button,.credential-dialog header button{display:grid;width:31px;height:31px;place-items:center;border-radius:8px;background: var(--cs-surface-subtle);cursor:pointer}.delivery-result{padding:var(--cs-space-16);border-bottom: 1px solid var(--cs-border)}.delivery-result strong,.delivery-result small{display:block}.delivery-result strong{margin-top:var(--cs-space-8);font-size:var(--cs-text-base)}.delivery-result small{margin-top:var(--cs-space-4);color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.delivery-detail>footer{display:grid;gap:var(--cs-space-8);padding:var(--cs-space-12) var(--cs-space-16)}.delivery-detail>footer small{color: var(--cs-text-muted);font-size:var(--cs-text-xs)}.dialog-backdrop{position:fixed;inset:0;z-index: var(--cs-z-dialog);display:grid;place-items:center;padding:var(--cs-space-20);background: var(--cs-scrim)}.credential-dialog{width:min(460px,100%);box-shadow: var(--cs-shadow-modal)}.credential-dialog form{display:grid;gap:var(--cs-space-12);padding:var(--cs-space-16)}.credential-dialog form>p{display:flex;gap:var(--cs-space-8);margin:0;padding:var(--cs-space-8);border-radius:8px;background: var(--cs-surface-accent-strong);color: var(--cs-text-brand-strong);font-size:var(--cs-text-xs)}.credential-dialog footer{display:flex;justify-content:flex-end;gap:var(--cs-space-8);padding-top:var(--cs-space-4)}
.notification-grid:has(.delivery-detail) .delivery-filter{grid-template-columns:1fr 1fr}.notification-grid:has(.delivery-detail) .delivery-filter select:last-of-type{grid-column:1/-1}
...
@media(max-width:780px){.connection-grid,.notification-grid,.notification-grid:has(.delivery-detail){grid-template-columns:1fr}.connection-detail,.delivery-detail{grid-row:1}.delivery-filter{grid-template-columns:1fr 1fr}.mapping-steps{grid-template-columns:1fr}.mapping-steps form{grid-template-columns:1fr}.proof{grid-column:auto}.table-panel thead{position:absolute;overflow:hidden;width:1px;height:1px;clip:rect(0 0 0 0)}.table-panel tbody,.table-panel tr,.table-panel td{display:block}.table-panel tr{padding:var(--cs-space-8) var(--cs-space-12);border-top: 1px solid var(--cs-border)}.table-panel td{display:grid;grid-template-columns:95px 1fr;padding:var(--cs-space-4) 0;border: 0}.table-panel td:before{content:attr(data-label);color: var(--cs-text-muted);font-size:var(--cs-text-xs);font-weight:var(--cs-weight-semibold)}.hero{grid-template-columns:40px 1fr}.hero>:last-child{grid-column:1/-1;justify-self:start}}
