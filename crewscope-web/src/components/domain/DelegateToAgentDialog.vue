...
import { buildConfigureReturn } from '../../app/configureReturn'
...
const props = defineProps<{
  ...
  /**
   * R42 “让 Agent 处理” intent from a WorkItem comment: the comment draft pre-fills the
   * objective so the member confirms it as this delegation's goal. Explicit latest intent
   * wins over an older form draft; nothing is submitted until the member confirms here.
   */
...
onMounted(async () => {
  restoreDraft()
  if (props.prefillNote) form.objective = props.prefillNote
...
/** Model-side preflight failures are configuration gaps; the rest need another kind of fix. */
const preflightConfigurable = computed(() => {
  const reason = String(preflightResource.value?.errorDetails?.reason ?? '')
  return reason === 'MODEL_BINDING_MISSING' || reason === 'DEFAULT_MISSING'
    || reason === 'DEFAULT_AMBIGUOUS' || reason === 'CONNECTION_FORBIDDEN'
})

/**
 * F02 configure-return: leave for the settings page carrying the work coordinates plus the
 * one-shot `delegate=coding` key, so coming back reopens this dialog with the draft restored.
 * Nothing is auto-submitted — returning only restores the form the member left.
 */
function goConfigure(target: 'model-settings' | 'repository-settings'): void {
  void router.push({
    name: target,
    query: {
      ...buildConfigureReturn('work', {
        team: props.codingScope.teamId,
        project: props.codingScope.projectId,
        workItem: props.workItem.id,
      }),
      delegate: 'coding',
    },
  })
    ...
    <div ref="dialog" class="delegate-dialog panel" role="dialog" aria-modal="true" aria-labelledby="delegate-title" tabindex="-1" @keydown="handleDialogKeydown">
      ...
      <section v-if="selectable.length" class="agent-selection" aria-labelledby="agent-selection-title">
        ...
        <div v-if="effectiveMode !== 'assign-only'" class="preflight-shell" aria-live="polite" aria-atomic="true">
          ...
          <div v-else-if="preflightResource?.phase === 'error'" class="preflight-state error" role="alert"><TriangleAlert :size="15" /><span>{{ preflightError() }}</span><BaseButton v-if="preflightConfigurable" type="button" size="small" variant="secondary" :disabled="submitting || retryable" @click="goConfigure('model-settings')">去配置模型并返回</BaseButton><BaseButton type="button" size="small" variant="secondary" :disabled="submitting || retryable" @click="runPreflight">重新预检</BaseButton></div>
      ...
      <template v-if="effectiveMode !== 'assign-only'">
        ...
        <div v-if="selectable.length" class="delegate-fields">
          <p v-if="prefillNote" class="prefill-note">执行目标已预填自你的评论草稿；确认或修改后才会提交，评论本身仍需单独发布才会留档。</p>
      ...
      <section v-if="selectable.length" class="intent-confirm" aria-labelledby="intent-title">
        ...
        <ul>
          ...
          <li v-if="defaultGaps.length" class="gap gap-action"><BaseButton type="button" size="small" variant="secondary" :disabled="submitting || retryable" @click="goConfigure('repository-settings')">去补配执行默认值并返回</BaseButton></li>
        </ul>
      </section>
