      ...
      <article v-for="association in associations" :key="association.task.id" class="conversation-task-card" :data-task-id="association.task.id">
        ...
        <div class="conversation-task-card__body">
          ...
          <p v-if="association.task.currentWaitingReason" class="conversation-task-card__waiting">
            等待原因 · {{ enumLabel(association.task.currentWaitingReason, taskExecutionWaitReasonLabels) }}
          </p>
...
<style scoped>
.conversation-tasks { max-width: 740px; margin: 0 auto var(--cs-space-12); overflow: hidden; border: 1px solid var(--cs-border-accent); border-radius: var(--cs-radius-md); background: var(--cs-surface-glass-strong); box-shadow: var(--cs-shadow-raised); }.conversation-tasks__header { display: flex; align-items: center; justify-content: space-between; gap: var(--cs-space-12); padding: var(--cs-space-12) var(--cs-space-12); border-bottom: 1px solid var(--cs-border); background: var(--cs-surface-accent); }.conversation-tasks__header p, .conversation-tasks__header h3 { margin: 0; }.conversation-tasks__header p { color: var(--cs-text-brand); font-size: var(--cs-text-xs); font-weight: var(--cs-weight-semibold); letter-spacing: .08em; text-transform: uppercase; }.conversation-tasks__header h3 { margin-top: var(--cs-space-2); font-size: var(--cs-text-sm); }.conversation-tasks__header h3 span { color: var(--cs-text-muted); font-weight: var(--cs-weight-semibold); }.conversation-tasks__header > span { color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.conversation-tasks :deep(.state-panel) { min-height: 120px; border: 0; border-radius: 0; }.conversation-tasks__list { display: grid; gap: var(--cs-space-2); background: var(--cs-border); }.conversation-task-card { display: grid; grid-template-columns: 32px minmax(0, 1fr) auto; align-items: start; gap: var(--cs-space-8); padding: var(--cs-space-12) var(--cs-space-12); background: var(--cs-surface); }.conversation-task-card__icon { display: grid; width: 32px; height: 32px; place-items: center; border-radius: 9px; background: var(--cs-agent-soft); color: var(--cs-agent); }.conversation-task-card__body { min-width: 0; }.conversation-task-card__title { display: flex; min-width: 0; align-items: flex-start; justify-content: space-between; gap: var(--cs-space-12); }.conversation-task-card__title strong { overflow: hidden; font-size: var(--cs-text-sm); line-height: var(--cs-leading-normal); text-overflow: ellipsis; white-space: nowrap; }.conversation-task-card__facts { display: flex; flex-wrap: wrap; gap: var(--cs-space-4) var(--cs-space-12); margin-top: var(--cs-space-4); color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.conversation-task-card__facts span { display: inline-flex; align-items: center; gap: var(--cs-space-4); }.live-fact { color: var(--cs-success); }.live-fact.connecting, .live-fact.reconnecting { color: var(--cs-warning); }.live-fact.error { color: var(--cs-danger); }.conversation-task-card__waiting { width: fit-content; margin: var(--cs-space-8) 0 0; padding: var(--cs-space-4) var(--cs-space-8); border-radius: 6px; background: var(--cs-warning-soft); color: var(--cs-warning); font-size: var(--cs-text-xs); }.conversation-task-card__origin { margin: var(--cs-space-8) 0 0; color: var(--cs-text-muted); font-size: var(--cs-text-xs); }.conversation-task-card__actions { display: flex; align-items: center; gap: var(--cs-space-4); }.conversation-tasks__error { display: flex; align-items: center; justify-content: center; gap: var(--cs-space-8); padding: var(--cs-space-8); margin: 0; background: var(--cs-danger-soft); color: var(--cs-danger); font-size: var(--cs-text-xs); }.conversation-tasks__error button { display: inline-flex; align-items: center; gap: var(--cs-space-4); color: inherit; font-size: inherit; font-weight: var(--cs-weight-semibold); cursor: pointer; }
@media (max-width: 767px) { .conversation-task-card { grid-template-columns: 30px minmax(0, 1fr); padding: var(--cs-space-12); }.conversation-task-card__icon { width: 30px; height: 30px; }.conversation-task-card__actions { grid-column: 2; }.conversation-task-card__actions :deep(button:first-child) { flex: 1; }.conversation-task-card__title { align-items: flex-start; }.conversation-task-card__title strong { white-space: normal; } }
</style>
