...
public final class TeamPersistenceMapper {
  ...
  /** Maps only the V6-sanctioned incomplete shape without weakening the Team aggregate. */
  public UninitializedTeam toUninitializedTeam(TeamEntity value) {
    if (value.ownerMemberId() != null || value.defaultWorkspaceId() != null) {
      throw new IllegalArgumentException("Team is already initialized");
    }
    return new UninitializedTeam(
        new TeamId(value.id()),
        new OrganizationId(value.organizationId()),
        value.name(),
        TeamStatus.valueOf(value.status()),
  ...
  public TeamMember toDomain(TeamMemberEntity value) {
    return TeamMember.reconstitute(
        ...
        new TeamScope(new OrganizationId(value.organizationId()), new TeamId(value.teamId())),
  ...
  public Principal toDomain(PrincipalEntity value) {
    Optional<ExternalIdentity> external =
        value.identityProvider() == null
            ? Optional.empty()
            : Optional.of(new ExternalIdentity(value.identityProvider(), value.externalSubject()));
