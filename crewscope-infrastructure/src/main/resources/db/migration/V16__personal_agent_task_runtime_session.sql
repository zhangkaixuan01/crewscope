-- TASK orchestration accepts either the member's Personal Agent or a Team Agent. The original
-- V10 constraint only encoded the Team Agent branch even though the domain allowed both.
...
ALTER TABLE crewscope.agent_runtime_session
    ADD CONSTRAINT ck_agent_runtime_session_shape CHECK (
        ...
        OR (session_purpose = 'TASK'
            ...
            AND ((agent_principal_type = 'PERSONAL_AGENT'
                    AND agent_profile_type = 'PERSONAL')
                OR (agent_principal_type = 'TEAM_AGENT'
