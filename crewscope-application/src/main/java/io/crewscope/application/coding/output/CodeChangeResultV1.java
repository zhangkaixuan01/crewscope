...
public record CodeChangeResultV1(
        ...
        @JsonProperty(required = true) @NotBlank @Pattern(regexp = SHA_256) String repositoryAnalysisHash,
        @JsonProperty(required = true) @NotBlank @Pattern(regexp = CANONICAL_UUID) String diffArtifactId,
        @JsonProperty(required = true) @NotBlank @Pattern(regexp = SHA_256) String diffArtifactHash,
        @JsonProperty(required = true) @NotBlank @Pattern(regexp = CANONICAL_UUID) String testEvidenceId,
        @JsonProperty(required = true) @NotBlank @Pattern(regexp = SHA_256) String testEvidenceHash,
