...
M4-S01 至 M4-S04、M4-D01 至 M4-D09、M4-I01 至 M4-I07 已完成，当前下一项为 M4-I08。M4-I01/I02 已将宿主 Git 收敛为类型化 Executor 与受管 Repository Resolver；M4-I03 已完成固定 Worktree 身份、双层路径锁、物理 Fingerprint、补偿回滚与幂等归档；M4-I04 已复用 AgentScope `DockerFilesystemSpec`、`DockerSandboxClient` 与 external Sandbox 注入，交付 CrewScope 持有的 TaskExecution 级容器生命周期、安全参数、资源预算以及 Lease/Fencing 调用窗口；M4-I05 已在该窗口上复用 `SandboxBackedFilesystem`，交付 8 个只读 Tool；M4-I06 继续复用同一 AgentScope 文件通道，交付 create/edit/patch/move/delete 五个受控写 Tool，并闭合 AllowedPaths、UTF-8、累计预算、大小写、符号链接和 TOCTOU 边界；M4-I07 已交付只接受 CommandKind 与有界选择器的结构化命令 Tool、四类 BuildProfile Runner、容器级超时进程树终止、日志 Artifact 与 CommandEvidence。验证见 [M4-I04 TaskExecution 级 Docker Sandbox](../testing/M4-I04-TaskExecution级Docker-Sandbox.md)、[M4-I05 受控 RepositoryInspectionTool](../testing/M4-I05-受控RepositoryInspectionTool.md)、[M4-I06 受控 CodingFilesystemTool](../testing/M4-I06-受控CodingFilesystemTool.md)与 [M4-I07 结构化 SandboxCommandTool 与 CommandEvidence](../testing/M4-I07-结构化SandboxCommandTool与CommandEvidence.md)。
