...
> 后续验收衔接（2026-09-19）：本文件保留 M9 已有证据及历史未执行事实。[M9b 计划](../plans/M9b-核心流程与使用体验收口.md) 负责修复主流程，并在新版本补齐技术验证。按用户要求取消真人测评，相关功能改由固定浏览器场景验证，不再要求招募用户；取消不等于旧测评通过。新证据附版本/时间，不追溯把旧版待执行项写成 PASS。当前 M9b 尚未实现。

M9-Q02 是 M9 的功能发布验收。门禁把开发机合同检查与真实后端/外部集成证据分开，避免把模拟结果误称为生产验证；后续不再要求真人参与测评。
...
| 首屏理解度（历史真人测评） | 原测评未执行；替代为 M9b 首屏待办/责任/主动作可见性及可达性断言 | 真人测评已取消；替代场景待执行 |
| Diff 评论可发现性（历史真人测评） | 原测评未执行；替代为 M9b 浏览器完成行评、固定目标/版本与失败恢复验证 | 真人测评已取消；替代场景待执行 |
| 配置路径 | 自动化/代理浏览器完成模型/仓库配置，验证配置返回与切 Team 草稿恢复 | 待执行，无需真人 |
