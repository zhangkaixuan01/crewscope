...
M6 已拆分为 50 个任务，覆盖 Activity、Inbox、Audit、影子投影重建、三流 Cursor、固定模板 Lark 通知、只读 Team Observer、OTel/Prometheus、部署、备份恢复、故障、负载和 MVP Release Gate。`M6-S01` 至 `M6-S05`、`M6-D01` 至 `M6-D09`、`M6-E01` 至 `M6-E07` 已完成，[ADR-020](../adr/ADR-020-投影代际重建与游标协议.md)、[ADR-021](../adr/ADR-021-三流恢复与前端合并协议.md)、[ADR-022](../adr/ADR-022-Inbox与固定模板通知授权协议.md) 和 [ADR-023](../adr/ADR-023-Team-Beta单机部署与发布验证协议.md) 已接受；下一任务为 `M6-I01`，实现 Activity、Inbox、Audit、Generation、DeadLetter 与运行健康的 PostgreSQL 查询 Adapter。
