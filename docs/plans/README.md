...
M7 已拆分为 39 个任务：4 个 Spike、8 个领域/迁移任务、8 个基础设施任务、7 个应用/API 任务、8 个前端任务和 4 个质量任务。范围包括单 Organization 自托管本地账号、可配置开放注册、Spring Session Redis、正式登录/注册页、首次 Team Onboarding、默认 Personal Agent、一次性邀请链接、Operator/监控凭证分离、认证固定攻击集和 V30→V32 升级门禁。M7-S01 至 M7-S04 已冻结认证技术、身份边界、密码防护和视觉基线；M7-D01 至 M7-D06 已完成 Account、Identity、Credential、Binding、Invitation 和安全事件领域契约；M7-D07/V31 已落地本地账号与身份表；M7-D08/V32 已落地 Digest-only TeamInvitation、跨 Scope 外键、一次性终态、完整接受链复核与并发停用互斥、过期扫描索引和元数据视图；M7-I01 至 M7-I07 已完成账号身份 Adapter、Redis Session、密码编码、Redis 登录防护、请求身份解析、邀请 Token 和 Bootstrap Operator 升级；M7-I08 已完成 Team Beta 正式/Demo 认证配置、Operator/Prometheus 凭证与安全链隔离、Registration/Cookie/Secret Guard、认证指标闭集、结构化日志脱敏和 V32 恢复合同。`M7-S01` 至 `M7-S04`、`M7-D01` 至 `M7-D08`、`M7-I01` 至 `M7-I08` 已完成，下一任务为 `M7-A01`。
