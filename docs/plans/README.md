...
- [M9b：核心流程与使用体验收口](M9b-核心流程与使用体验收口.md)
  - [S01 核心流程与合同冻结](../spikes/M9b-S01-核心流程与合同冻结.md)：2026-09-22 已完成，全部 D01–D08、17 项决定、接口/权限/查询细表、ADR-038 和可重跑 Spike 证据已交付，不代表功能全部实现。
  - [F01 安全命令与请求隔离完成记录](../testing/M9b-F01-安全命令与请求隔离完成.md)：2026-09-22 父包已关闭；意图级显式重试、页面/Store 隔离、飞书原目标与原事实核实完成。171 文件/1029 项前端、13 项后端定向、2 项隔离 HTTP 浏览器及质量门禁通过。
  - [A01 创建编号、恢复与基本编辑](../testing/M9b-A01-创建编号恢复与基本编辑完成.md)：2026-09-22 实现与自动化验证完成；三类创建准确定位/刷新恢复、普通与 TaskIntent 自动编号、项目代号与基本编辑接通，见[正式接口](../api/M9b-创建结果与命令恢复契约.md)。174 文件/1053 项前端、领域696/应用695项、真实 PG/新 JVM 重启浏览器链路与质量门禁通过。下一包 A02，未提交/推送；[后端首批](../testing/M9b-A01-持久创建结果首批.md)保留历史。
  - [A02 GitHub 连接绑定与导入恢复](../testing/M9b-A02-GitHub连接绑定与导入恢复完成.md)：2026-09-23 实现与定向验证完成；默认路径不要求 Account ID、Allowlist 或 RepositoryKey，按真实身份、所选仓库和原 job 坐标恢复；[连续配置契约](../api/M9b-GitHub连续配置契约.md)为权威增量说明。已交接 A03，未提交/推送。
  - [A03 模型连接健康与重新启用](../testing/M9b-A03-模型连接健康与重新启用完成.md)：2026-09-23 实现与定向自动化验证完成；SUSPENDED 可健康检查，只有当前凭据 HEALTHY 才能重新启用，REVOKED 不可恢复；接通版本/幂等/权限、凭据隔离、禁止重定向与模型限制/未知价格语义。[模型服务管理契约](../api/M9b-模型服务管理契约.md)为权威接口说明。
  - [A04 项目构建配置与支持矩阵](../testing/M9b-A04-项目构建配置与支持矩阵完成.md)：2026-09-23 实现与定向自动化验证完成；Java v1 hash/快照保持兼容，Node/npm v2 只开放受控 PREPARE/COMPILE/TEST，项目默认值不要求填写内部 Key 或宿主命令；真实 Node 样本、镜像 digest 和跨架构证据留在 M9b-Q02。按串行主线下一包为 F05；A05 待 A07 完成后开始，未提交/推送。
  - 历史批次：[安全提交首批](../testing/M9b-F01-安全提交首批实现.md)、[Scope 隔离与飞书目标固定先行批](../testing/M9b-F01-Scope隔离与飞书目标固定.md)，保留当时 962/994 项测试记录，不覆盖当前完成状态。
  - [M9b 实现边界与验收契约](M9b-实现边界与验收契约.md)：跨包交接、表单默认、导航/权限映射、路由返回与主动作状态、筛选/草稿边界、固定反例和开工前决策；与主计划 §11 的切片样例/否决条件一起读取，避免实施自由解释。
  - [全菜单布局与交互细节 Review](../reviews/M9b-全菜单布局与交互细节Review.md)：15+9 菜单与 23 页的位置/顺序/首屏/窄屏子要求 L01–L12，归入原 R01–R43；含逐菜单目标与自动化布局断言，不新增父包或真人验收。
...
**当前 M9b（进行中，2026-09-23 A04 已完成）**：依据 [M9 后全流程 Review](../reviews/M9后-全流程使用体验与竞品对照Review.md)、[菜单逐操作 Review](../reviews/M9b-菜单逐操作产品Review.md) 与 [Multica 迁移 Review](../reviews/M9b-Multica用户迁移体验Review.md)，用 15 个主线工作包关闭 R01–R43（17 + 15 + 7 + 4 项）。覆盖首用/执行/审查/配置、身份隔离和成员撤权，并补编辑、模型启用、项目构建默认值、审计/导入恢复、共享内容阅读、个人视图与发送意图；Inbox 升级可恢复收纳。23 页及 15 个一级、9 个二级菜单逐操作和迁移场景采用自动化/代理浏览器验证，不设真人测评门槛；父包数量不变不代表工作量不变。M9b 内部按串行主线执行：`S01 → F01 → A01 → A02 → A03 → A04 → F05 → A06 → A07 → A05 → F02 → F03 → F04 → Q01 → Q02`，之后才进入 `M10-S01`。M9-Q02 未完成的技术验证由 M9b-Q02 在新版本上补证并互链，不继承历史真人测评要求，也不追溯改写旧测试结论。A04 的 Node 真实样本、镜像 digest 与跨架构证据仍由 Q02 补证，不把占位实验 profile 当成发行支持。成员生命周期基础写侧及 ADR-038 提前归 M9b-A07，M11-D02 仅负责新增 WebSocket/在场通道的撤权接入。
...
| ADR-038 | [团队成员生命周期与责任转移](../adr/ADR-038-团队成员生命周期与责任转移.md) | M9b-S01 已接受基础设计，A07 实现；M11-D02 增补 WebSocket/在场撤权 |
