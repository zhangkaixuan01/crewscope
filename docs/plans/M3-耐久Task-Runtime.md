...
| ID | 类型 | 依赖 | 涉及模块 | 实施内容 | 验证 |
|---|---|---|---|---|---|
| `M3-D01` | TASK | M2-A07 | domain/application | 已完成：实现 Task、ConversationTaskLink、TaskSource 和 `TaskResponsibilitySnapshot`；Task 保存受约束的来源 WorkItem 和可复现输入引用，定义从 WorkItem/Conversation 创建、当前有效尝试与业务状态生命周期 | [M3-D01 Task 领域模型](../testing/M3-D01-Task领域模型.md)；18 个专项测试覆盖 Scope 闭合、来源唯一性、一个 WorkItem 的多个 Task、双向关联、责任快照完整性、当前尝试切换、失败后新尝试、取消和不可变创建事实；M3 不引入 TaskDefinition 表 |
| `M3-D02` | TASK | D01, ADR-001 | domain/application | 已完成：实现 TaskExecution、attempt/maxAttempts、parentExecutionId、调度优先级、notBefore、安全失败分类和完整尝试状态机；使用 WAITING + 独立原因，分离 Task 业务状态、执行状态、控制请求与终态事实 | [M3-D02 TaskExecution 状态机](../testing/M3-D02-TaskExecution状态机.md)；14 个专项测试覆盖 READY、WAITING(RUNTIME)、CLAIMED、PREPARING、RUNNING、WAITING、PAUSE/CANCEL 请求、PAUSED、RECOVERING、完成、失败、单链重试、maxAttempts、乐观锁、非法回退与终态不可变 |
| `M3-D03` | TASK | D02, S02 | domain/application | 已完成：实现 StepExecution、PlanVersion、PlanStep、PolicySnapshot、SafetyEnforcementOverlay 版本引用与父哈希链、执行 Principal、检查点和 Todo 摘要；TaskExecution 使用乐观锁切换当前 Policy、Overlay 和 Plan，Plan 候选只有通过当前权限交集与结构校验后才能发布 | [M3-D03 Step、Plan 与 Policy 领域模型](../testing/M3-D03-Step-Plan与Policy领域模型.md)；27 个专项测试覆盖 Step 串行状态、计划/策略版本不可变、父版本、Hash 防篡改、权限扩大、当前 Policy/Overlay、实时撤权收紧、Executor 闭合、检查点单调性与边界、Todo 映射、失败重试、可选 Step 跳过和无 Step Lease 约束 |
| `M3-D04` | TASK | M2-I02 | domain/application | 已完成：实现 Organization/环境隔离的 ExecutionRuntime Registry 事实、RuntimeWorker 稳定身份、RuntimeCapabilities 共享能力词汇、`all/worker` RuntimeProfile、显式 Worker 状态、容量快照和心跳事实；Drain 停止新 Claim，心跳新鲜度作为派生事实 | [M3-D04 Runtime 与 Worker 领域模型](../testing/M3-D04-Runtime与Worker领域模型.md)；17 个专项测试覆盖 Runtime/Worker 注册、稳定 Key 格式与 Repository 唯一查询契约、能力/语言/构建系统匹配、能力收缩对账、容量上下界、启停、Drain、过期心跳、乐观锁和跨 Organization/环境/Runtime 隔离 |
| `M3-D05` | TASK | D02,D04,S01 | domain/application | 已完成：实现唯一 TaskExecution `ExecutionLease`、一次性 `ClaimReceipt`、Claim Token/Hash、TaskExecution 单调 Fencing Token、完整 `LeaseOwnership` 坐标、有界 `PREPARE/RUN` Lease、独立 Lease Heartbeat/Version、过期和释放原因；Repository Port 显式约束 Claim、阶段切换、续租与释放语义 | [M3-D05 ExecutionLease 与所有权协议](../testing/M3-D05-ExecutionLease与所有权协议.md)；18 个新增专项测试覆盖 Claim、Token 脱敏/Hash、Fencing 单调、续期、阶段切换、时长上下界、重建时间线、显式释放、过期边界、旧 Fencing Token、错误 execution/attempt/Worker/Runtime/Token、终态互斥和只存在 TaskExecution Lease |
| `M3-D06` | TASK | D04,D05 | domain/application | 已完成：实现 TaskCredentialGrant、TaskTokenClaims、TaskTokenGrantScope、JTI Hash、最小 Provider/Tool/显式资源范围、签发、使用、撤销、过期和 Repository Port | [M3-D06 Task Token 与 CredentialGrant](../testing/M3-D06-Task-Token与CredentialGrant.md)；14 个专项测试覆盖 Claims 闭合、最小范围、过期、撤销、错误 Lease/Execution/attempt/Runtime/Worker/Claim/Fencing/Binding/Tool/Capability/资源、重复 JTI Hash、乐观锁、重建不变式和安全字段脱敏 |
| `M3-D07` | TASK | D02,D03,D06 | domain/application | 已完成：保留 M2 Conversation Session 不变量并增加 TASK/STEP/SPECIALIST TaskAgentRuntimeSession；实现 AgentRun、多 Segment、AgentInterrupt、Resume Receipt、continuity gap、RuntimeArtifact 和 AgentStateSnapshot 元数据及 Repository Port | [M3-D07 Agent 运行与恢复领域模型](../testing/M3-D07-Agent运行与恢复领域模型.md)；11 个专项测试覆盖 Session 与 Task/Execution/Step/AgentProfile 闭合、同一 Step 多次 AgentRun、Run/Segment 序号、Pending 唯一 Repository 契约、Resume 幂等、快照身份与 Hash、终态不可变和大结果仅保存 Artifact 引用 |
| `M3-D08` | TASK | D01..D07 | infrastructure | 已完成：新增 `V10__durable_task_runtime.sql`，建立 Task、链接、Execution、Step、Plan、PolicySnapshot、责任快照、Runtime、Worker、Lease、CredentialGrant、AgentRun、Interrupt、RuntimeArtifact、AgentStateSnapshot 表及 Task/Step AgentRuntimeSession 扩展 | [M3-D08 V10 耐久 Task Runtime 数据迁移](../testing/M3-D08-V10-耐久Task-Runtime数据迁移.md)；6 个 PostgreSQL 专项场景覆盖空库/V1→V10、V9→V10、非默认 `search_path`、复合 Scope 外键、审计与技术时间字段、状态形状、READY/Lease 索引、Plan/Policy 父版本、Session Scope、部分唯一索引和禁止 Step Lease |
| `M3-D09` | TASK | D08 | infrastructure | 已完成：实现 M3 JPA Entity、Mapper、17 个 Repository Port 和 13 个 Adapter，以及 Claim、Heartbeat、Phase、Release、Sweeper 和 READY Queue JDBC 协议；固定完整 Keyset Cursor、`SKIP LOCKED`、外层锁事务、确定性 Session 身份校验与条件更新边界 | [M3-D09 Task Runtime 持久化与 JDBC 协议](../testing/M3-D09-Task-Runtime持久化与JDBC协议.md)；6 个 PostgreSQL 场景覆盖领域往返、Provider/Runtime JSONB、快照、乐观锁、队列排序、Queue/Lease 锁跳过、正常/过期释放、Session 碰撞、条件更新、Cursor、查询计划、跨 Scope 和事务回滚 |
