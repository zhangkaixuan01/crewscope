...
> 当前进度：`M6-S01` 至 `M6-S05` 已完成，ADR-020 至 ADR-023 已接受；下一任务为 `M6-D01`（2026-08-25）
...
| ID | 类型 | 依赖 | 涉及模块 | 实施内容 | 验证 |
|---|---|---|---|---|---|
| `M6-D01` | TASK | S01,S02 | domain/application | 实现 ActivityEvent、ActivityVisibility、公开 Payload Schema、Subject/Actor/Reference、TeamSequence、ProjectionGeneration 和 Cursor Scope；WorkItem/Team 查询共享同一事件身份 | Payload 白名单、可见性、Schema 演进、同 Team 单调序号、跨 Team/Organization 拒绝、Cursor 篡改和历史事件恢复测试 |
| `M6-D02` | TASK | S03,D01 | domain/application | 实现 InboxSource、InboxItem、InboxDisposition、类型/优先级/截止时间、来源 Revision、关闭原因和成员 READ/ACTED/ARCHIVED 强版本命令；投影与成员处置分离 | 重复来源、责任变化、Review 失效、Confirmation 终结、异常关闭、重建保留处置、陈旧 ETag 和跨成员越权测试 |
| `M6-D03` | TASK | S03,M5-D08,D09 | domain/application | 实现 NotificationTemplate/Version、变量 Schema、Preference、Intent、AuthorizationSnapshot、`NOTIFY_COLLABORATION` 参数、Delivery/Receipt 状态机、去重和再次投递命令；扩展保持 M5 GitHub Action Confirmation 不变量不变 | 任意文本、未知变量、旧模板、收件人/Binding/Policy 漂移、重复 Dispatch、超时、最终失败、历史不可变及 GitHub 无确认仍零写入测试 |
| `M6-D04` | TASK | S04,ADR-004,ADR-006 | domain/application | 实现 LarkExternalTenant、LarkMemberMapping、映射验证来源/版本/状态、Collaboration Recipient、Connection 能力和 Repository Port | 禁止显示名模糊绑定，Tenant/User 唯一、跨 Scope、映射撤销、Connection 撤销、迟到验证和敏感字段边界测试 |
| `M6-D05` | TASK | M5-D01..D05 | domain/application | 定义 `team-observer@1`、默认 Team Service Principal、默认禁用的只读 Team AgentProfile、TeamSummaryRequest/Result、允许工具和数据范围不变量 | 每 Team 唯一、未配置不可启用、TEAM Model Binding、只读工具精确集合、结构化摘要、成员可见性、无写权限和迁移恢复测试 |
| `M6-D06` | TASK | S01,M0-D06 | domain/application | 扩展 Audit 查询契约：稳定 EventCategory/Outcome、Initiator/Actor/Agent、Provider/Correlation 引用、脱敏摘要、保留级别和有界导出请求；M6 不实现 AuditEvent 删除 | Audit 只追加、未知 Payload 失败关闭、敏感字段脱敏、Team Admin 权限、时间/游标分页和导出上限测试 |
| `M6-D07` | TASK | S01 | domain/application | 实现 ProjectionDefinition、Generation、RebuildJob、ValidationResult、切换状态机、Checkpoint/DeadLetter 引用和管理员命令不变量 | 单活 Generation、失败不可切换、旧 Worker 禁写、取消、重试、版本冲突和 Audit/DomainEvent 安全投影测试 |
| `M6-D08` | TASK | D01..D04,D06,D07 | infrastructure | 新增 `V27__activity_inbox_notification.sql`，落地 Activity、Inbox 来源/处置、Notification Template/Preference/Intent/Delivery、Projection Generation/Rebuild/DeadLetter 与 Audit 查询索引 | 空库、V26→V27、非默认 Schema、约束、索引、跨租户 FK、处置保留、代际切换和清理重放测试 |
| `M6-D09` | TASK | D04,D05,D08 | infrastructure | 新增 `V28__lark_mapping_and_team_observer.sql`，落地 Lark Tenant/Member Mapping，并为既有 READY Team 确定性补齐 Service Principal、`team-observer@1` 与 `DISABLED` Team Observer Profile；不生成 ModelConnection/Configuration | 空库、V27→V28、重复迁移、稳定 ID、部分旧 Team、映射唯一性、未配置不可运行、Agent/Profile/Principal 关系和回滚测试 |
