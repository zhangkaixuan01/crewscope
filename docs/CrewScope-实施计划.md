...
> 当前进度：M0、M1、M2、M3 已完成；M4-S01 至 M4-S04、M4-D01 至 M4-D09、M4-I01 至 M4-I12、M4-A01 至 M4-A07 已完成，下一项为 M4-F01（2026-08-20）
...
M4 已拆分为 44 个可执行任务，在 M3 执行内核上交付 RepositoryBinding、CodingTargetSnapshot、ExecutionWorkspace、Git Worktree、Docker Sandbox、受控代码工具、Coding Specialist、DiffArtifact、TestEvidence、Repository 管理与 Execution Studio。`M4-S01` 至 `M4-S04`、`M4-D01` 至 `M4-D09`、`M4-I01` 至 `M4-I12`、`M4-A01` 至 `M4-A07` 已完成，下一项为 `M4-F01`。M4-I01 至 I10 已完成 Git、Workspace、Sandbox、受控 Tool、Diff、Artifact 与启动恢复基础设施；M4-I11 已完成固定 Skill/Tool 面的原生 Coding Specialist、Plan/Todo、Compaction、Eviction、AgentState 安全点与严格输出；M4-I12 已完成 Specialist Step 协调、策略修复预算、事件优先 Checkpoint、Pause/Resume/Cancel、Snapshot/Workspace 恢复、权威结果复验与终态映射；M4-A01 已完成 RepositoryBinding 管理、管理员授权、幂等命令、强版本并发、Preflight 和路径脱敏 API；M4-A02 已完成 Control/Conversation 统一 CodingTarget 委托、完整 Scope 与精确 BuildProfile 校验、基线 Preflight、幂等 Hash 和首个 attempt 前的原子 Snapshot 固化；M4-A03 已完成 Durable Worker Workspace 生命周期、跨 Worker 文件写预算、Maven 测试证据发布与平台权威 Coding 结果固化；M4-A04 已完成当前/历史 Coding attempt、Workspace/Sandbox、Diff Manifest、Command/TestEvidence 和耐久 Coding Result 的成员查询 API、Scope 绑定 Cursor 与固定次数 DTO 投影；M4-A05 已完成 Workspace、Diff RESET/DELTA、TestEvidence 与最终 DiffArtifact 的统一耐久 Task Timeline、SSE 续传、持续授权和终态关流；M4-A06 已完成 Patch、构建日志和测试报告的成员受权 Range/分页 API、类型化下载、响应预算、并发限制和安全审计；M4-A07 已完成 Workspace 容量、Sandbox/Watcher/清理健康的 Runtime Fleet/Actuator 投影，以及平台管理员 Reconcile/Archive 命令、幂等 Receipt 和耐久审计。验证见 [M4-I10 Worker 启动资源对账](testing/M4-I10-Worker启动资源对账.md)、[M4-I11 AgentScope Coding Specialist 运行时](testing/M4-I11-AgentScope-Coding-Specialist运行时.md)、[M4-I12 Coding Specialist Step 执行与恢复](testing/M4-I12-Coding-Specialist-Step执行与恢复.md)、[M4-A01 RepositoryBinding 管理与 Preflight API](testing/M4-A01-RepositoryBinding管理与Preflight-API.md)、[M4-A02 CodingTarget 委托与原子固化](testing/M4-A02-CodingTarget委托与原子固化.md)、[M4-A03 Coding Workspace 执行生命周期](testing/M4-A03-Coding-Workspace执行生命周期.md)、[M4-A04 Coding attempt 查询 API](testing/M4-A04-Coding-attempt查询API.md)、[M4-A05 Coding 事件历史与 SSE](testing/M4-A05-Coding事件历史与SSE.md)、[M4-A06 Coding Artifact 内容 API](testing/M4-A06-Coding-Artifact内容API.md)与 [M4-A07 Runtime Fleet 与运维命令](testing/M4-A07-Runtime-Fleet与运维命令.md)。
