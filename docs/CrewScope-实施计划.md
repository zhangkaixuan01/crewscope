...
本文件保存 M0–M8 的范围、依赖、周期和出口门槛。可执行 Backlog 与架构决策分别维护：
...
| M10 | Agent 执行前自动检索团队规范与代码库知识并留下可追溯的引用证据，成功套路可沉淀为 Skill 被后续任务复用 | 8–10 周 |
| M11 | 多名成员同屏协作并看到彼此在场与实时变更，WorkGraph 呈现依赖与关键路径，外部 MCP 工具可受控接入，成员角色变更与移除成为一等领域命令（责任链不留无主项），中英双语与桌面端发布 | 8–10 周 |
| M12 | 单机自托管与 K8s 多 Organization 托管双形态并行，企业身份、配额计量与 SLO 治理就位 | 10–12 周 |
...
M9 起的周期以同一小组为基准，但结构不同：M9 是前端主导里程碑，前端投入占比应显著高于此前任何里程碑；M10 是后端与 Agent 主导；M11、M12 为 Feature 级基线，实际周期在各自 Spike 完成后重估。M9 与 M10 存在硬顺序依赖（M10 的知识管理、Skill 审核、分段 Timeline 三块前端直接消费 M9 的设计系统与交互内核），不建议调换。生产级 Kubernetes 执行拓扑、Vault/KMS 高可用、企业身份、容灾和长期 SLO 加固收敛到 M12。
