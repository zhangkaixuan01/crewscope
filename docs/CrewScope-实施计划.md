...
本文件保存 M0–M12（含补充收口 M9b）的里程碑范围、依赖、周期和出口门槛；M9–M12 的细化以各自计划为准。可执行 Backlog 与架构决策分别维护：
...
- [M8：产品化与工程收口执行清单](plans/M8-产品化与工程收口.md)；
- [M9b：核心流程与使用体验收口执行清单](plans/M9b-核心流程与使用体验收口.md)；
- [CrewScope 前端设计规范](CrewScope-前端设计规范.md)；
- [Architecture Decision Records](adr/README.md)。

业务建设里程碑使用 0.5–2 天的 TASK；M8 作为产品化、重构和发布收口里程碑，使用少量 3–10 个工作日的完整工作包，内部通过 Checklist 和小提交推进，不为单个类、页面、脚本或测试继续编号。
...
flowchart LR
  ...
  M7 --> M8["M8 产品化与工程收口"]
  M8 --> M9["M9 产品体验重构与设计系统"]
  M9 --> M9b["M9b 核心流程与使用体验收口"]
  M9b --> M10["M10 Agent 智能跃迁与知识闭环"]
  M10 --> M11["M11 协作规模化与开放生态"]
  M11 --> M12["M12 企业化与多租户治理"]
```

M0–M8 为「能力建设」阶段，目标是把 CrewScope 做到可发行；M9 起进入「产品力」阶段，依据 [M8 后全面架构与产品体验 Review](reviews/M8后-全面架构与产品体验Review.md) 的结论，先补齐被跳过的前端设计系统与交互内核地基（M9），再向 Agent 能力纵深与团队知识沉淀推进（M10），最后解决多人协作规模化（M11）与企业化交付（M12）。

2026-09-19 [M9 后全流程 Review](reviews/M9后-全流程使用体验与竞品对照Review.md) 发现核心路径仍有断点，因此在 M9 与 M10 之间加入 **M9b（2026-09-22 已启动 S01，整体进行中）**：15 个主线包覆盖 R01–R43（17 + 15 + 7 + 4 项），按 23 页用户场景及 [15 个一级、9 个二级菜单逐操作清单](reviews/M9b-菜单逐操作产品Review.md) 验收。优先 HTTP/编号/GitHub 首用、身份隔离、阅读/审查正确性与飞书目标固定，再收口配置、统一工作区、搜索/批处理和成员撤权。[Multica 迁移 Review](reviews/M9b-Multica用户迁移体验Review.md) 另补共享内容语义、个人视图、发送意图、真实模型限制，升级 Inbox 可恢复收纳，并加入固定迁移场景的自动化/代理浏览器验证，不设真人测评门槛。新增问题在原父包拆切片并重估，不以样式变化声称迁移优势。M9 历史成果保留，未完成技术验证在 M9b 新版本补证，历史真人测评要求不再继承；成员生命周期基础从 M11 提前，M11 只新增实时通道接线。

当前进度：[M9b-S01 核心流程与合同冻结](spikes/M9b-S01-核心流程与合同冻结.md)、[M9b-F01 安全命令与请求隔离](testing/M9b-F01-安全命令与请求隔离完成.md)、[A01 创建编号、恢复与基本编辑](testing/M9b-A01-创建编号恢复与基本编辑完成.md) 均已完成实现与自动化验证（2026-09-22）。A01 已接通三类创建原子结果/受权查询/刷新恢复、自动编号/项目代号及五字段基本编辑；174文件/1053项前端、领域696/应用695项、真实 PG 与新 JVM 重启浏览器链路通过。下一顺序 A02，M9b 整体仍进行中；工作区未提交/推送，未恢复已停掉的部署。
...
| M8 | 新 Team 通过 Setup Center 完成执行配置，项目形成可发行、可观测、可维护的产品版本 | 5–7 周 |
| M9 | 登录后首屏直接呈现"我的工作"看板、进度与待办；基础组件层与反馈闭环（Toast/统一确认/Tooltip/骨架屏）建成；Diff 具备语法高亮、行号、分栏与行级 Review 评论；列表支持排序、批量操作与相对时间，刷新不丢筛选与视图偏好；状态流转从"下拉选枚举 + 提交"改为"点状态徽章即出动作"、看板拖拽与 ⌘K 三条路径（操作步数 7 → ≤ 2，成功后有撤销窗口，不可执行的动作直接给出原因与补救办法），前端状态机由领域 `ALLOWED_TRANSITIONS` 生成而非手抄；各种配置的展示与交互重构（字号可读、未保存不丢、参数边界可见、Revision 可对比、配置项可搜、凭证可核对、九个配置入口收敛到统一 Settings 外壳并有常驻配置健康页）；全站 218 处禁用控件全部能说明"为什么不能点"；14 个菜单在手机与窄窗口下全部可达且页面主操作不消失（当前 12 个不可达）、每条路由在浏览器标签里有自己的名字、观察类页面看完有下一步、选人不再要求粘 UUID、Activity 筛选下推到服务端既有参数、根路径落点从空对话框改为个人工作台；全站排版符合设计规范，⌘K 命令面板与全局快捷键可用，面板可拖拽、长列表虚拟滚动、暗色主题上线，前端 DTO 由 OpenAPI 契约生成 | 8–10 周 |
| M9b | 无必填内部 Key、可靠提交/重试、空环境模型/仓库/构建配置、一次委托、统一审查交付、成员撤权与真实使用验收 | S01 按 15 个主线包与实际人员估算 |
| M10 | 单 Agent 知识检索、引用、Skill 复用与成本质量；多 Agent 独立可选 | S01 后按主线重估 |
| M11 | Web 实时协作、WorkGraph、成员撤权的新通道接入；MCP、i18n、桌面独立选入 | S01 后按选入范围重估 |
| M12 | 单机组织设置、OIDC、原子配额、数据治理与基础 SLI；托管/K8s/其他身份协议可选 | 主线与扩展分别估算 |

建议周期以 2 名后端与 1 名前端小组为基准，M0–M6 用 18–24 周交付 Team Beta，M7 追加 3–4 周完成开放用户入口，M8 使用 5–7 周完成产品化与工程收口。单人实施按 M0 到 M8 串行推进，M8 可按“配置闭环”和“工程生产收口”两条线交错执行。

M9 起的周期按实际团队与选入范围估算，不把可选扩展全部打包成固定工期。M9b 以已交付 M9 代码开始修复，M10 正式依赖 M9b-Q02，M11/M12 依赖前一里程碑主线而非其全部可选项。默认四服务、源码构建和一键启动不变；各 S01 细化主线与选入扩展的预算及证据。统一范围、依赖和 ADR 规划见 [M10–M12 计划规则](plans/README.md#9-m10m12-计划规则)。
...
把 M0–M7 已完成的 Team Beta 能力收口为更容易配置、更容易部署、更容易维护和更适合公开发行的产品版本。M8 不新增业务领域，重点解决新 Team 配置路径分散、核心代码职责过重、构建依赖与配置漂移、正式镜像不可直接消费、Trace/告警/备份未形成生产闭环以及质量反馈成本偏高的问题。

### 14.2 产品配置闭环

- 服务端以现有模型、Agent、WorkProject、Repository、Provider 和 Runtime 权威事实派生 Team Setup Readiness，不建立第二套状态；
- 就绪状态按 Personal Conversation、普通团队任务、Coding/Review、GitHub Draft PR、飞书通知和 Team Observer 分能力计算；
- 必需项与可选项分开，GitHub 或飞书缺失不能阻断基础对话和站内工作；
- Web 提供统一 `/setup` Setup Center、Today 摘要和 Onboarding 后续入口；
- 每项只发布闭合状态、安全原因码、处理权限和受信任站内动作，不发布 Secret、Endpoint、Provider 原始错误或任意 URL。

### 14.3 工程与生产收口

- 按稳定职责拆分 WorkPage/Task Store、Notification Intent Projection、Review Persistence、Operations API 和 AgentScope Task Runtime，保持 API、事件、数据库、幂等和恢复语义兼容；
- 收口 Maven 直接依赖，默认构建移除未使用 Kubernetes/Fabric8 运行依赖，生成 Spring Configuration Metadata，并自动检查环境变量、Secret、Compose 和文档漂移；
- 建立 Tag 驱动的 GHCR 正式发行，Backend/Web 共享 Git Revision，发布固定 Digest、SBOM、Provenance 和签名；
- 使 Trace 可查询或明确关闭，补齐 Prometheus 告警、自动备份/保留/恢复、HSTS/CSP/TLS 和本地 Compose Loopback 边界；
- 前端 Coverage 默认纳入全部生产业务代码，CI 分为 PR、Main 和 Release 三层，并按变更范围提供快速而不漏检的反馈。

### 14.4 任务与验收

M8 使用 9 个完整工作包，不再沿用 M0–M7 的细粒度拆分：

1. `M8-S01` 冻结产品化、Readiness、发行和质量合同；
2. `M8-A01` 交付 Team Setup Readiness 服务端纵切；
3. `M8-F01` 交付 Setup Center 与引导体验；
4. `M8-E01` 完成核心职责与 Port 合同收口；
5. `M8-I01` 完成依赖、配置元数据和本地开发边界治理；
6. `M8-I02` 完成 GHCR 正式发行与开源发布材料；
7. `M8-I03` 完成 Trace、告警、自动备份和 TLS 运维加固；
8. `M8-Q01` 重建 Coverage、Lint、架构和分层 CI 门禁；
9. `M8-Q02` 在正式发行产物上关闭 M8 Release Gate。

M8 最终必须证明：新用户不需要手工改库即可从注册、首 Team 和 Setup Center 进入有效执行；基础能力不被可选集成阻断；正式镜像可验证、可部署、可观测、可备份和可恢复；M0–M7 固定攻击、故障、迁移、浏览器和恢复门禁零回归。完整范围、依赖、工作包和 Release Gate 见 [M8 执行清单](plans/M8-产品化与工程收口.md)。

### 14.5 非目标

Kubernetes 高可用、多 Organization SaaS、企业 OIDC/SAML/MFA/SCIM、Vault/KMS 高可用、Plugin 市场、PR 自动合并、生产发布、Autopilot、Desktop 和 Mobile 不进入 M8。
...
| Setup Readiness 与真实配置状态漂移 | 只在查询时从权威事实派生，不建立可写 Readiness 表；以能力级状态、稳定原因码和版本坐标拒绝旧快照 | M8-A01 出口 |
...
- [M8 执行清单](plans/M8-产品化与工程收口.md)：9 个较大的 SPIKE/FEATURE/TASK/HARDENING 工作包，覆盖 Team Setup Readiness、Setup Center、核心职责重构、依赖与配置治理、GHCR 正式发行、Trace/告警/自动备份/TLS、分层 CI 和 M8 Release Gate。
