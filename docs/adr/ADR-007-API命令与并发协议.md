...
> 状态：ACCEPTED<br>
> 日期：2026-08-07<br>
> 更新：2026-09-22（M9b-A01 追加受权持久创建结果；保留 M7-A07 单值 Header 与重放协议）<br>
> 影响里程碑：M0–M7、M9b
...
### M9b 已接受增量（A01 后端首批已实现，前端恢复待接线）

2026-09-22 S01 冻结 [创建结果与恢复方案](../spikes/M9b-S01-核心流程与合同冻结.md#3-d01命令身份结果查询与编号)：旧四字段回执保持兼容，新增受权 command-results 查询；结果坐标与业务/回执同事务持久化，恢复必须原 actor 和当前对象权限。未查到可披露结果不证明原命令未发生，旧回执不猜测回填，不增加自动幂等 TTL。A01/F01 的正式 API 和实现必须遵守该增量，不能把以下旧回执字段当作已包含结果坐标。

首批实现见[正式接口契约](../api/M9b-创建结果与命令恢复契约.md)与[验证记录](../testing/M9b-A01-持久创建结果首批.md)：V40 为项目/普通工作项/对话创建保存不可变坐标，受权 GET 仅从原 Idempotency-Key Header 定位；所有结果响应 no-store，统一 404 不披露不存在/他人/撤权差异。跨刷新 UI、自动编号及其他命令类型尚未接通，不因新增表和接口宣告 A01 完成。
