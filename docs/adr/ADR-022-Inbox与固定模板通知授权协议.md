...
2026-09-22 S01 冻结 [可逆收纳与迁移方案](../spikes/M9b-S01-核心流程与合同冻结.md#5-d04inbox-可逆状态迁移与批次)：F04 新版允许显式 UNREAD/正版本、ARCHIVED 恢复为 READ；版本仍只增、不删权威行，源业务与通知去重不回退。必须同时更新 Java 状态转换、V27 后追加的约束/触发器迁移、未读计数/工作台与重放。下文 M6 已实现的单向收纳描述是旧版基线，不覆盖该目标合同；新状态落库后不承诺旧二进制无损回退。
