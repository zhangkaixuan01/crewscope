...
command -v docker >/dev/null || { echo "Docker is required for the M1 release gate." >&2; exit 1; }
command -v pnpm >/dev/null || { echo "pnpm is required for the M1 release gate." >&2; exit 1; }
...
# Keep documentation, backend contracts and browser behavior in the same blocking gate.
