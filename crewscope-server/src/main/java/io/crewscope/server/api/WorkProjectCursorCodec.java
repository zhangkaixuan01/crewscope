...
/** Versioned opaque transport codec for the WorkProject updated-time/ID keyset position. */
public final class WorkProjectCursorCodec {
  ...
  public String encode(WorkProjectCursor cursor) {
    WorkProjectCursor source = Objects.requireNonNull(cursor, "cursor");
  ...
  public WorkProjectCursor decode(String token) {
    if (token == null
        || token.isBlank()
    ...
    try {
      ...
      return new WorkProjectCursor(UtcTimestamp.from(instant), new WorkProjectId(id));
